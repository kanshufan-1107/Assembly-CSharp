...
public class DALA_Dungeon_Boss_59h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_59h_Female_Pandaren_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID != "DALA_Chu" && playerHeroCardID != "DALA_Vessina" && playerHeroCardID != "DALA_Squeamlish")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_59h_Female_Pandaren_StartOfGame_01);
			break;
		case 102:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_59h_Female_Pandaren_Misc_01);
			break;
		case 103:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_59h_Female_Pandaren_Misc_02);
			break;
		case 104:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPower);
			break;
		case 105:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerTreasure);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "DALA_BOSS_59t"))
		{
			...
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_59h_Female_Pandaren_BossFruitNinja_01);
			}
		}
		else
		{
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossLotusBruiser);
		}
	}
}
