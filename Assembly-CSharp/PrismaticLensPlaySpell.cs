...
public class PrismaticLensPlaySpell : Spell
{
	...
	{
		int taskCount = FindTaskCountToRunUntilDraw();
		if (taskCount > 0)
		{
			yield return StartCoroutine(CompleteTasksUntilDraw(taskCount));
	...
	{
		m_swapSpell = SpellManager.Get().GetSpell(m_CostSwapSpell);
