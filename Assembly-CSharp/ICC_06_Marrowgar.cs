...
public class ICC_06_Marrowgar : ICC_MissionEntity
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 101:
			if (m_BoneSpikeLines.Count != 0)
			{
				...
				string randomBoneSpikeLine = m_BoneSpikeLines[Random.Range(0, m_BoneSpikeLines.Count)];
				m_BoneSpikeLines.Remove(randomBoneSpikeLine);
				yield return PlayLineOnlyOnce(enemyActor, randomBoneSpikeLine);
		...
		case 102:
			...
			{
				string randomBoneStormLineInitial = m_BoneStormLines[Random.Range(0, m_BoneStormLines.Count - 1)];
				yield return PlayMissionFlavorLine(enemyActor, randomBoneStormLineInitial);
			}
			break;
		case 103:
			if (m_BoneStormLines.Count != 0)
			{
				string randomBoneStormLine = m_BoneStormLines[Random.Range(0, m_BoneStormLines.Count)];
				yield return PlayMissionFlavorLine(enemyActor, randomBoneStormLine);
			}
			break;
		case 104:
	...
	protected override IEnumerator HandleGameOverWithTiming(TAG_PLAYSTATE gameResult)
	{
		...
		{
			yield return new WaitForSeconds(5f);
			string voLine = "VO_ICC08_LichKing_Male_Human_Intro_02.prefab:0e52794eab68e6345b9ecc37992e08d9";
