...
public class DragRigidBody : FsmStateAction
{
	...
	{
		if (!isDragging && InputCollection.GetMouseButtonDown(0))
		{
			if (!Physics.Raycast(_cam.ScreenPointToRay(InputCollection.GetMousePosition()), out var hit, 100f) || !hit.rigidbody || hit.rigidbody.isKinematic)
			{
				return;
			}
			StartDragging(hit);
	...
	private void Drag()
	{
		...
		Ray ray = _cam.ScreenPointToRay(InputCollection.GetMousePosition());
		if (_goPlane != null)
		{
			Plane _plane = default(Plane);
