...
public class GIL_Dungeon_Boss_29h : GIL_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_29h_Female_Satyr_Intro_01.prefab:7ec7479acf5acb74b9e4302ff92736e0", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_29h_Female_Satyr_EmoteResponse_01.prefab:05b465e15cf38234aa54811463501d0d", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		{
			string randomLine = PopRandomLineWithChance(m_TunesLines);
			if (randomLine != null)
			{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()))
		{
			...
			{
				string ranLine = m_SmallMinionLines[Random.Range(0, m_SmallMinionLines.Count)];
				m_SmallMinionLines.Remove(ranLine);
