...
public class GIL_Dungeon_Boss_48h : GIL_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			string randomLine = m_RandomIntroLines[Random.Range(0, m_RandomIntroLines.Count)];
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(randomLine, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_48h_Female_Gnome_EmoteResponse_01.prefab:7a5cb51ef214a804fae3c933aae63b57", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()))
		{
			...
			{
				string ranLine = m_RandomBossTimeWarpLines[Random.Range(0, m_RandomBossTimeWarpLines.Count)];
				m_RandomBossTimeWarpLines.Remove(ranLine);
				yield return PlayLineOnlyOnce(enemyActor, ranLine);
			}
		}
	}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()))
		{
			...
			{
				string ranLine = m_RandomPlayerTimeWarpLines[Random.Range(0, m_RandomPlayerTimeWarpLines.Count)];
				m_RandomPlayerTimeWarpLines.Remove(ranLine);
				yield return PlayLineOnlyOnce(enemyActor, ranLine);
			}
		}
	}
	...
	protected override IEnumerator HandleStartOfTurnWithTiming(int turn)
	{
		...
		case 2:
			...
			yield return PlayLineOnlyOnce(playerActor, "VO_GILA_900h_Female_Gnome_EVENT_NEMESIS_TURN2_02.prefab:bc3fdc2e58660c2488283fe53df5a36a");
		...
		case 8:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineOnlyOnce(playerActor, "VO_GILA_900h_Female_Gnome_EVENT_NEMESIS_TURN6_01.prefab:dff37aa4d569154418338aab31cdaf95");
	...
	protected override IEnumerator RespondToResetGameFinishedWithTiming(Entity entity)
	{
		...
		if (entity.GetControllerSide() == Player.Side.FRIENDLY)
		{
			...
			if (!(text == "GILA_900p"))
			{
				...
				{
					string ranLine = m_RandomPlayerAltTimeLines[Random.Range(0, m_RandomPlayerAltTimeLines.Count)];
					m_RandomPlayerAltTimeLines.Remove(ranLine);
					yield return PlayLineOnlyOnce(enemyActor, ranLine);
				}
			}
			else
			{
				string randomLine = PopRandomLineWithChance(m_RandomPlayerHeroPowerLines);
				if (randomLine != null)
				{
		...
		{
			string randomLine2 = PopRandomLineWithChance(m_RandomBossAltTimeLines);
			if (randomLine2 != null)
			{
