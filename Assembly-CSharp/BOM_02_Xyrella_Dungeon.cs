...
public class BOM_02_Xyrella_Dungeon : BOM_02_Xyrella_MissionEntity
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		float m_XyrellaSpeakerChanceRoll = Random.Range(0f, 1f);
		switch (missionEvent)
		{
		case 1000:
		...
		case 1011:
			...
			foreach (string currentLine3 in m_PlayerVOLines)
			{
		...
		case 58034:
			...
			{
				if (m_XyrellaSpeakerChanceRoll > 0.5f)
				{
					yield return MissionPlayVO(friendlyActor, m_Xyrella_HPHeal);
				}
				else
				{
		...
		case 58036:
			...
			if (shouldPlayBanterVO())
			{
				if (m_XyrellaSpeakerChanceRoll > 0.5f)
				{
					yield return MissionPlayVO(friendlyActor, m_Xyrella_HPRez);
				}
				else
				{
