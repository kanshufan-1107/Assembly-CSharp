...
public class ICC_10_Deathwhisper : ICC_MissionEntity
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 102:
			...
			{
				GameState.Get().SetBusy(busy: true);
				string randomHeroPowerLine = m_deathwhisperHeroPowerLines[Random.Range(0, m_deathwhisperHeroPowerLines.Count)];
				m_deathwhisperHeroPowerLines.Remove(randomHeroPowerLine);
				yield return PlayLineOnlyOnce(enemyActor, randomHeroPowerLine);
				GameState.Get().SetBusy(busy: false);
			}
			break;
		case 103:
	...
	protected override IEnumerator HandleGameOverWithTiming(TAG_PLAYSTATE gameResult)
	{
		...
		{
			yield return new WaitForSeconds(5f);
			string voLine = "VO_ICC10_LichKing_Male_Human_Lose_01.prefab:e0877aec1d0153442a1735f55a907989";
	...
	private Actor GetValithria()
	{
		...
		foreach (Card card in player.GetBattlefieldZone().GetCards())
		{
			...
			if (cardEntity.GetControllerId() == player.GetPlayerId() && cardEntity.GetCardId() == "ICCA10_001")
			{
