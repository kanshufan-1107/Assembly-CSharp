...
public class ULDA_Dungeon_Boss_32h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_32h_Female_Dryad_IntroSpecial_Finley_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "ULDA_Elise")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_32h_Female_Dryad_IntroResponseElise_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (missionEvent == 101)
		{
			yield return PlayBossLine(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_TurnOne_01);
		}
		else
		{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_190"))
		{
			if (cardID == "ULD_719")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_PlayerTrigger_Desert_Hare_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_PlayerTrigger_Pit_Crocolisk_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "DAL_733":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_BossTriggerDreamwayGuardians_01);
				break;
			case "GVG_032":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_BossTriggerGroveTender_01);
				break;
			case "EX1_166":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_32h_Female_Dryad_BossTriggerKeeperoftheGrove_01);
				break;
			}
		}
	}
