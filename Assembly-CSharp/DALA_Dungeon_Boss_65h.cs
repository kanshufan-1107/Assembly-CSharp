...
public class DALA_Dungeon_Boss_65h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			case "DALA_DALA_Eudora":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_65h_Male_HighElf_IntroEduora_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_Rakanishu":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_65h_Male_HighElf_IntroRakanishu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_Squeamlish":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_65h_Male_HighElf_IntroSqueamlish_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_Tekahn":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_65h_Male_HighElf_IntroTekahn_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			}
			if (playerHeroCardID != "DALA_George" && playerHeroCardID != "DALA_Chu")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerStopDeathLines);
			break;
		case 102:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerTriggerLines);
			break;
		case 103:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_PlayerGainArmorLines);
			break;
		case 104:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_65h_Male_HighElf_PlayerAlexstraza_01);
			break;
		case 105:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_65h_Male_HighElf_BossEyeforanEye_01);
			break;
		case 106:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_65h_Male_HighElf_PlayerPlatedScarabDeathrattle_01);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "EX1_360"))
		{
			if (cardID == "EX1_354")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_65h_Male_HighElf_BossLayOnHands_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_65h_Male_HighElf_BossHumility_01);
		}
	}
}
