...
public class InGameMessageAction : IProtoBuf
{
	...
	{
		if (!(obj is InGameMessageAction other))
		{
			return false;
		}
		...
		if (HasAction != other.HasAction || (HasAction && !Action.Equals(other.Action)))
		{
			return false;
		}
		if (HasViewCounts != other.HasViewCounts || (HasViewCounts && !ViewCounts.Equals(other.ViewCounts)))
		{
			return false;
		}
	...
	{
		InGameMessageAction instance = new InGameMessageAction();
	...
	public uint GetSerializedSize()
	{
		...
		{
			size++;
			size += ProtocolParser.SizeOfUInt64((ulong)Action);
		}
		if (HasViewCounts)
		{
			size++;
			size += ProtocolParser.SizeOfUInt64((ulong)ViewCounts);
		}
		if (HasUid)
		{
			size++;
