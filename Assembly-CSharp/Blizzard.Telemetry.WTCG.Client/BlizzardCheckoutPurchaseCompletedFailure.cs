...
public class BlizzardCheckoutPurchaseCompletedFailure : IProtoBuf
{
	...
	public override int GetHashCode()
	{
		...
		foreach (string i in ErrorCodes)
		{
			hash ^= i.GetHashCode();
		}
		return hash;
	...
	{
		if (!(obj is BlizzardCheckoutPurchaseCompletedFailure other))
		{
			return false;
		}
		...
		if (ErrorCodes.Count != other.ErrorCodes.Count)
		{
			return false;
		}
		for (int i = 0; i < ErrorCodes.Count; i++)
		{
			if (!ErrorCodes[i].Equals(other.ErrorCodes[i]))
			{
				return false;
			}
		}
	...
	{
		BlizzardCheckoutPurchaseCompletedFailure instance = new BlizzardCheckoutPurchaseCompletedFailure();
	...
	public static void Serialize(Stream stream, BlizzardCheckoutPurchaseCompletedFailure instance)
	{
		...
		foreach (string i6 in instance.ErrorCodes)
		{
			stream.WriteByte(50);
	...
	public uint GetSerializedSize()
	{
		...
		{
			foreach (string i6 in ErrorCodes)
			{
				size++;
				uint byteCount6 = (uint)Encoding.UTF8.GetByteCount(i6);
