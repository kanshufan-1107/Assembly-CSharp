...
public class ThirdPartyPurchaseCompletedFail : IProtoBuf
{
	...
	{
		if (!(obj is ThirdPartyPurchaseCompletedFail other))
		{
			return false;
		}
		...
		if (HasBpayProvider != other.HasBpayProvider || (HasBpayProvider && !BpayProvider.Equals(other.BpayProvider)))
		{
			return false;
		}
		if (HasErrorInfo != other.HasErrorInfo || (HasErrorInfo && !ErrorInfo.Equals(other.ErrorInfo)))
		{
			return false;
		}
		return true;
	...
	{
		ThirdPartyPurchaseCompletedFail instance = new ThirdPartyPurchaseCompletedFail();
	...
	public uint GetSerializedSize()
	{
		...
		{
			size++;
			uint byteCount5 = (uint)Encoding.UTF8.GetByteCount(BpayProvider);
			size += ProtocolParser.SizeOfUInt32(byteCount5) + byteCount5;
		}
		if (HasErrorInfo)
		{
			size++;
			uint byteCount6 = (uint)Encoding.UTF8.GetByteCount(ErrorInfo);
