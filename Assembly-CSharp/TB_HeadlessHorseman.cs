...
public class TB_HeadlessHorseman : MissionEntity
{
	...
	private void GetHorsemanHead()
	{
		...
		if (headID != 0)
		{
			...
			if (headEnt != null)
			{
				isHeadInPlay = headEnt.GetZone() == TAG_ZONE.PLAY;
			}
			if (headEnt != null)
			{
	...
	{
		int horsemanId = GameState.Get().GetGameEntity().GetTag(GAME_TAG.TAG_SCRIPT_DATA_ENT_2);
		Entity horsemanEnt = GameState.Get().GetEntity(horsemanId);
		if (horsemanId != 0)
		{
			if (horsemanEnt != null)
			{
				horsemanCard = horsemanEnt.GetCard();
				isHorsemanInPlay = horsemanEnt.GetZone() == TAG_ZONE.PLAY;
			}
