...
public class DRGA_Evil_Fight_10 : DRGA_Dungeon
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 118:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_16h_Male_Elemental_Evil_Fight_10_Misc_01_01);
			}
			break;
		case 119:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_16h_Male_Elemental_Evil_Fight_10_Misc_02_01);
			}
			break;
		case 120:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_16h_Male_Elemental_Evil_Fight_10_Rakanishu_Transform_01);
			}
			break;
		case 122:
		...
		case 129:
			if (!m_Heroic)
			{
				yield return PlayLineInOrderOnce(friendlyActor, m_VO_DRGA_BOSS_16h_Male_Elemental_Evil_Fight_10_Rakanishu_Elemental_HeroPowerLines);
			}
			break;
		case 130:
			if (!m_Heroic)
			{
				yield return PlayLineInOrderOnce(friendlyActor, m_VO_DRGA_BOSS_16h_Male_Elemental_Evil_Fight_10_Rakanishu_DragonForm_HeroPowerLines);
			}
			break;
		case 151:
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		case "ULD_238":
			if (m_Heroic)
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_12h_Male_Human_Evil_Fight_10_Karl_LeagueOfExplorers_01);
			}
			break;
		}
	}
	...
	public override void OnPlayThinkEmote()
	{
		...
		if (thinkEmoteBossThinkChancePercentage > randomThink)
		{
			Actor enemyActor = GameState.Get().GetOpposingSidePlayer().GetHeroCard()
				.GetActor();
			if (opposingHeroCard == "DRGA_BOSS_11h2")
			{
				string voLine = PopRandomLine(m_GeorgeBossIdleLinesCopy);
			...
			{
				string voLine2 = PopRandomLine(m_KarlBossIdleLinesCopy);
