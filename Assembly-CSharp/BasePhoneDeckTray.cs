...
public abstract class BasePhoneDeckTray : MonoBehaviour
{
	...
	{
		if (!UniversalInputManager.Get().InputIsOver(base.gameObject) && !m_cardsContent.MouseIsOverDeckHelperButton(Box.Get().GetCamera()))
		{
	...
	protected virtual void OnCardCountUpdated(int cardCount, int maxCount)
	{
		string cardLabel = string.Empty;
		string cardCountText = string.Empty;
		if (cardCount > 0)
		{
			...
			{
				cardLabel = GameStrings.Get("GLUE_DECK_TRAY_CARD_COUNT_LABEL");
				cardCountText = GameStrings.Format("GLUE_DECK_TRAY_COUNT", cardCount, CollectionManager.Get().GetDeckSize());
			}
		}
	...
	protected void ShowDeckBigCard(DeckTrayDeckTileVisual cardTile, float delay = 0f)
	{
		...
		long cardDbId = GameUtils.TranslateCardIdToDbId(entityDef.GetCardId());
		m_deckBigCard.SetCreatorName(GetCreatorNameFromChildCardDbId(cardDbId));
	...
	private string GetCreatorNameFromChildCardDbId(long cardId)
	{
		...
		if (!CardIdToCreatorMap.TryGetValue(cardId, out var creatorId))
		{
			return string.Empty;
		}
		CardDbfRecord record = GameDbf.Card.GetRecord((int)creatorId);
		if (record == null)
		{
			return string.Empty;
		}
