...
public class TGTArrow : MonoBehaviour
{
	...
	public void FireArrow(bool randomRotation)
	{
		if (randomRotation)
		{
			Vector3 meshRot = m_ArrowMesh.transform.localEulerAngles;
			m_ArrowMesh.transform.localEulerAngles = new Vector3(meshRot.x + Random.Range(0f, 360f), meshRot.y, meshRot.z);
	...
	public void ArrowAnimation()
	{
		...
		Vector3 pos = m_ArrowRoot.transform.localPosition;
		iTween.MoveFrom(m_ArrowRoot, iTween.Hash("position", new Vector3(pos.x, pos.y, pos.z + 0.4f), "islocal", true, "time", 0.05f, "easetype", iTween.EaseType.easeOutQuart));
	}
