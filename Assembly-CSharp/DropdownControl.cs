...
public class DropdownControl : PegUIElement
{
	...
	public bool removeItem(object value)
	{
		...
		DropdownMenuItem dropdownMenuItem = m_items[itemIndex];
	...
	public void setSelectionToLastItem()
	{
		...
		if (m_items.Count != 0)
		{
			...
			DropdownMenuItem lastItem = m_items[m_items.Count - 1];
			lastItem.SetSelected(selected: true);
			m_selectedItem.SetValue(lastItem.GetValue(), m_itemTextCallback(lastItem.GetValue()));
		}
	}
	...
	public void setSelectionToFirstItem()
	{
		...
		if (m_items.Count != 0)
		{
			...
			DropdownMenuItem firstItem = m_items[0];
			firstItem.SetSelected(selected: true);
			m_selectedItem.SetValue(firstItem.GetValue(), m_itemTextCallback(firstItem.GetValue()));
		}
	}
	...
	public void setSelection(object val)
	{
		...
		{
			DropdownMenuItem item = m_items[i];
			object itemVal = item.GetValue();
			if ((itemVal == null && val == null) || itemVal.Equals(val))
			{
				item.SetSelected(selected: true);
				m_selectedItem.SetValue(itemVal, m_itemTextCallback(itemVal));
			}
			else
			{
	...
	{
		hideMenu();
		object prevSelection = getSelection();
		object selection = item.GetValue();
		setSelection(selection);
		m_itemChosenCallback(selection, prevSelection);
	}
	...
	private void layoutMenu()
	{
		...
		OrientedBounds menuItemBounds = TransformUtil.ComputeOrientedWorldBounds(m_menuItemTemplate.gameObject);
		if (menuItemBounds != null)
		{
			float menuItemHeight = menuItemBounds.Extents[1].magnitude * 2f;
			...
			{
				TransformUtil.SetLocalScaleToWorldDimension(m_menuMiddle, new WorldDimensionIndex(menuItemHeight * (float)(m_items.Count - 1), 2));
			}
			m_menu.UpdateSlices();
		}
	}
