...
public class KARRunes : MonoBehaviour
{
	...
	{
		if (InputCollection.GetMouseButtonUp(0) && IsOver(m_BookPageR) && !m_isAnimating)
		{
			FlipBookPages();
		}
		if (InputCollection.GetMouseButtonUp(0) && IsOver(m_BookPageL) && !m_isAnimating)
		{
			FlipBookPages(isRight: false);
		}
		if (InputCollection.GetMouseButtonUp(0) && IsOver(m_FloorRune) && !m_isAnimating)
		{
	...
	private void FlipBookPages(bool isRight = true)
	{
		...
		{
			string pageSound = m_PageFlipSounds[Random.Range(0, m_PageFlipSounds.Count - 1)];
			if (pageSound != null)
			{
				SoundManager.Get().LoadAndPlay(pageSound);
			}
		}
		if (isRight)
		{
