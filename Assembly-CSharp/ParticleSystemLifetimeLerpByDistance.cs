...
public class ParticleSystemLifetimeLerpByDistance : MonoBehaviour
{
	...
	private void Update()
	{
		float distance = Vector3.Distance(base.transform.position, targetObject.transform.position);
		...
		foreach (ScaledObject scaledObject in array)
		{
			...
			main.startLifetime = Mathf.Lerp(scaledObject.startLifetimeMin, scaledObject.startLifetimeMax, (Mathf.Clamp(distance, scaledObject.minDistance, scaledObject.maxDistance) - scaledObject.minDistance) / (scaledObject.maxDistance - scaledObject.minDistance));
		}
	}
}
