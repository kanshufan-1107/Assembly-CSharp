...
public class AchieveDbfRecord : DbfRecord
{
	...
	[SerializeField]
	private Achieve.LeagueType m_leagueType = Achieve.ParseLeagueTypeValue("unknown");
	...
	public Achieve.LeagueType LeagueType => m_leagueType;
	...
	public override void SetVar(string name, object val)
	{
		...
		case "LEAGUE_TYPE":
			if (val == null)
			{
				m_leagueType = Achieve.LeagueType.UNKNOWN;
			}
			else if (val is Achieve.LeagueType || val is int)
			{
				m_leagueType = (Achieve.LeagueType)val;
			}
			else if (val is string)
			{
				m_leagueType = Achieve.ParseLeagueTypeValue((string)val);
			}
			break;
	...
	public override Type GetVarType(string name)
	{
		return name switch
		{
			...
			"LEAGUE_TYPE" => typeof(Achieve.LeagueType), 
	...
	{
		AchieveDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(AchieveDbfAsset)) as AchieveDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
