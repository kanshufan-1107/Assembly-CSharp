...
public class PackLogo : CustomWidgetBehavior
{
				...
				else
				{
					...
					using (AssetHandle<GameObject> assetHandle = ShopUtils.LoadStorePackPrefab(m_displayedPack))
					{
						...
						case LogoTextureType.MiniSet:
							...
							break;
						case LogoTextureType.SellableDeck:
							textureName = storePackDef.GetSellableDeckTextureName();
							break;
						}
						...
						using (loadedTexture)
						{
							if (!loadedTexture)
							{
								if ((bool)data)
								{
									...
									if (!AssetLoader.Get().LoadAsset(textureName, onTextureLoaded, false, AssetLoadingOptions.DisableLocalization))
									{
										onTextureLoaded(textureName, null, false);
									}
								}
								else
								{
					...
					m_renderer.enabled = false;
					if (!AssetLoader.Get().LoadAsset(textureName, onTextureLoaded, true, (!m_localizeTexture) ? AssetLoadingOptions.DisableLocalization : AssetLoadingOptions.None))
					{
						onTextureLoaded(textureName, null, true);
					}
					callback(null);
				}
			}
	...
	private BoosterDbId GetDesiredPack()
	{
		if (m_useDataModel)
		{
			if (GetDataModel(25, out var model) && model is PackDataModel packDataModel)
			{
				packDataModel.AlternateBoosterId = CatalogUtils.GetAlternateBoosterId(packDataModel.Type);
				return packDataModel.Type;
			}
			if (GetDataModel(15, out model))
			{
				switch (m_textureType)
				{
				case LogoTextureType.MiniSet:
				{
					MiniSetDbfRecord record = MiniSetLayout.GetDbfRecord((ProductDataModel)model);
					if (record != null)
					{
						return (BoosterDbId)record.BoosterId;
					}
					break;
				}
				case LogoTextureType.SellableDeck:
				{
					int rewardId = ((ProductDataModel)model).Items[0].ItemId;
					SellableDeckDbfRecord sellableDeckRecord = GameDbf.SellableDeck.GetRecord(rewardId);
					if (sellableDeckRecord != null)
					{
						return (BoosterDbId)sellableDeckRecord.BoosterId;
					}
					break;
				}
				}
