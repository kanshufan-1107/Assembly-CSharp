...
public class WidgetInstance : Widget, INestedReferenceResolver, IAsyncInitializationBehavior, IPopupRendering
{
	...
	private bool m_popupRenderingApplied;
	...
	public bool WillPreload
	{
		get
		{
			bool isParentTickingInactive = m_parentWidgetTemplate != null && m_parentWidgetTemplate.WillTickWhileInactive;
			if (m_loadingPolicy != LoadingPolicy.LoadAlways && !m_requestedPreload)
			{
				if (isParentTickingInactive)
				{
	...
	public override InitializationState InitState
	{
		get
		{
			if (Widget == null)
			{
				return InitializationState.NotStarted;
			}
			return Widget.InitState;
		}
	}
	...
	public event Action WidgetLoadFailed;
	...
	private void Start()
	{
		...
		WidgetRunner.s_inactiveInstances.Remove(this);
	}
	...
	public void Deinitialize()
	{
		if (Widget == null)
		{
			m_pendingDataContext.UnbindAllDataModels();
		}
	...
	{
		WidgetRunner.s_inactiveInstances.Add(this);
		if (!m_startedInitialization)
		{
	...
	public NestedReferenceComponentInfo GetComponentInfoById(long id)
	{
		if (Widget == null)
		{
			return new NestedReferenceComponentInfo(null, componentsChecked: false);
		}
		return new NestedReferenceComponentInfo(Widget.GetComponentById(id));
	}
	...
	private void HandleWidgetTemplateInstantiated(object unused)
	{
		...
		if (m_popupRoot != null)
		{
			PropagatePopupRendering(m_popupRoot);
		}
	}
	...
	private void HandleWidgetTemplateLoadFailed()
	{
		if (Widget == null)
		{
			Log.UIFramework.PrintError("WidgetInstance " + base.name + " failed to load its prefab " + ((!string.IsNullOrEmpty(m_widgetTemplate.AssetString)) ? m_widgetTemplate.AssetString : "ASSET REFERENCE MISSING") + ".\n Check that the widget prefab with this GUID isn't missing from the project or your widget(s) will be sad :(");
			DeferredWidgetBehaviorInitialization = false;
			m_wasInitialized = false;
			base.gameObject.SetActive(value: false);
			this.WidgetLoadFailed?.Invoke();
		}
	}
	...
	public override void BindDataModel(IDataModel dataModel, bool overrideChildren = false)
	{
		if (dataModel == null)
		{
			AssetReference assetRef = AssetReference.CreateFromAssetString(m_widgetTemplate.AssetString);
			Log.UIFramework.PrintError("WidgetInstance " + base.name + " attempted to bind null dataModel Prefab:" + ((!string.IsNullOrEmpty(assetRef)) ? assetRef.FileName : "ASSET REFERENCE MISSING") + " GO:" + ((!string.IsNullOrEmpty(base.gameObject.name)) ? base.gameObject.name : "GAME OBJECT MISSING") + " " + $"Instance:{GetInstanceID()}.");
		}
		else if (Widget == null)
		{
	...
	public static WidgetInstance Create(string assetString, bool deferred = false)
	{
		...
		WidgetInstance instance = obj.AddComponent<WidgetInstance>();
		instance.m_widgetTemplate.AssetString = assetString;
	...
	public void EnablePopupRendering(IPopupRoot popupRoot)
	{
		...
		{
			m_popupRenderingApplied = false;
		}
		if (!m_popupRenderingApplied)
		{
			if (InitState == InitializationState.Done)
			{
				PropagatePopupRendering(popupRoot);
			}
			else
			{
				RegisterReadyListenerDoOnce(PropagatePopupRendering, popupRoot);
			}
		}
		m_popupRoot = popupRoot;
	}

	private void PropagatePopupRendering(object payload)
	{
		if (payload is IPopupRoot popupRoot)
		{
			popupRoot.ApplyPopupRendering(base.transform, m_popupRenderingComponents, m_layerOverride > 0, m_layerOverride, InitState == InitializationState.Done && (ParentWidgetTemplate == null || !ParentWidgetTemplate.IsDesiredHiddenInHierarchy));
			m_popupRenderingApplied = true;
		}
	}
