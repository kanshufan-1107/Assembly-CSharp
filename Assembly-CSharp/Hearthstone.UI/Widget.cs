...
public abstract class Widget : MonoBehaviour, IAsyncInitializationBehavior, IStatefulWidgetComponent, ILayerOverridable
{
	...
	public abstract InitializationState InitState { get; }
	...
	protected void RegisterReadyListenerDoOnce(Action<object> listener, object payload = null, bool callImmediatelyIfReady = true)
	{
		m_readyState.RegisterSetListener(listener, payload, callImmediatelyIfReady, doOnce: true);
	}
	...
	{
		if (GetDataModel((typeof(T).GetField("ModelId", BindingFlags.Static | BindingFlags.Public).GetValue(null) as int?).Value, out var dataModel))
		{
	...
	public static string GetObjectDebugName(object obj)
	{
		...
		return $"{unityObj.name} ({unityObj.GetInstanceID()})";
	}
