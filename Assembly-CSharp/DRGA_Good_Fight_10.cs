...
public class DRGA_Good_Fight_10 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_BossStartHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_EmoteResponseHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "UNG_960":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_Player_LostintheJungle_01);
				break;
			case "LOOT_541":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_Player_Togwaggle_01);
				break;
			case "UNG_950":
				GameState.Get().SetBusy(busy: true);
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_Player_Vinecleaver_01);
				GameState.Get().SetBusy(busy: false);
				break;
			case "CS2_142":
			case "DAL_614":
			...
			case "TOT_033":
			case "ULD_184":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_11h_Male_Human_Good_Fight_10_Player_Kobold_01);
				break;
			}
		}
	}
