...
public class AdventureDungeonCrawlTreasureOption : AdventureOptionWidget
{
	...
	private void PlaySpells(List<SpellDisplayData> spellTypes)
	{
		...
		foreach (SpellDisplayData spellData in spellTypes.Where((SpellDisplayData r) => r.m_SpellType != SpellType.NONE).ToList())
		{
			Spell spell = m_cardActor.GetSpell(spellData.m_SpellType);
			if (spell == null)
			{
				Debug.LogErrorFormat("AdventureDungeonCrawlTreasureOption.PlaySpells - {0} spell type was null!", spellData.m_SpellType);
			}
			else
			{
				spell.SetLocalPosition(spellData.m_RelativeLocation);
	...
	public void Init(long cardDbId, bool locked, string lockedText, bool upgraded, bool completed, bool newlyUnlocked, OptionAcknowledgedCallback acknowledgedCallback)
	{
		...
		InitWidget(cardName, locked, lockedText, upgraded, completed, newlyUnlocked, acknowledgedCallback);
		Rollout();
	}
	...
	{
		base.Select();
		if (!(m_selectedCallback is TreasureSelectedOptionCallback selectedCallback))
		{
