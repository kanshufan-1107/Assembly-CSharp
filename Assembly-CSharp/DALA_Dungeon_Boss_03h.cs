...
public class DALA_Dungeon_Boss_03h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_03h_Male_Goblin_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID != "DALA_Chu" && playerHeroCardID != "DALA_Vessina")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossSpell);
			break;
		case 102:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_03h_Male_Goblin_BossPresto_04);
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "DALA_BOSS_03t":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossReductomara);
				break;
			case "DALA_BOSS_03t2":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossBunnifitronus);
				break;
			case "DALA_BOSS_03t3":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossPresto);
				break;
			case "DALA_BOSS_03t4":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossYoggers);
				break;
			case "GIL_147":
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_03h_Male_Goblin_BossFireSpell_01);
				break;
			}
		}
	}
