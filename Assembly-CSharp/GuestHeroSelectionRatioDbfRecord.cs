...
public class GuestHeroSelectionRatioDbfRecord : DbfRecord
{
	...
	{
		GuestHeroSelectionRatioDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(GuestHeroSelectionRatioDbfAsset)) as GuestHeroSelectionRatioDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
