...
public class DALA_Dungeon_Boss_64h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_64h_Male_Orc_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_Chu")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_64h_Male_Orc_IntroChu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_64h_Male_Orc_Exposition_01);
			break;
		case 102:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_64h_Male_Orc_Exposition_02);
			break;
		case 103:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_64h_Male_Orc_BossBuffedMinionDies_01);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "DAL_088"))
		{
			if (cardID == "CFM_940")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_64h_Male_Orc_BossIKnowAGuy_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_64h_Male_Orc_BossSafeguard_01);
		}
	}
}
