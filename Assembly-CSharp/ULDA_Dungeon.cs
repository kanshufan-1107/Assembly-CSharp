...
public class ULDA_Dungeon : ULDA_MissionEntity
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 801:
			if (!(ChanceToPlayDeathPlotTwistVOLine() < chanceRoll) && TurnOfPlotTwistLastPlayed < currentTurn)
			{
				TurnOfPlotTwistLastPlayed = currentTurn;
				if (isFinalBoss)
				{
					yield return PlayRandomLineAlways(enemyActor, m_BossDeathPlotTwistTauntLines);
					break;
				}
				TurnOfPlotTwistLastPlayed = currentTurn;
			...
			break;
		case 802:
			if (!(ChanceToPlayMadnessPlotTwistVOLine() < chanceRoll) && TurnOfPlotTwistLastPlayed < currentTurn)
			{
				TurnOfPlotTwistLastPlayed = currentTurn;
				if (isFinalBoss)
				{
					yield return PlayRandomLineAlways(enemyActor, m_BossMadnessPlotTwistTauntLines);
				}
				else
				{
			...
			break;
		case 803:
			if (!(ChanceToPlayMurlocPlotTwistVOLine() < chanceRoll) && TurnOfPlotTwistLastPlayed < currentTurn)
			{
				TurnOfPlotTwistLastPlayed = currentTurn;
				if (isFinalBoss)
				{
					yield return PlayRandomLineAlways(enemyActor, m_BossMurlocPlotTwistTauntLines);
				}
				else
				{
			...
			break;
		case 804:
			if (!(ChanceToPlayWrathPlotTwistVOLine() < chanceRoll) && TurnOfPlotTwistLastPlayed < currentTurn)
			{
				TurnOfPlotTwistLastPlayed = currentTurn;
				if (isFinalBoss)
				{
					yield return PlayRandomLineAlways(enemyActor, m_BossWrathPlotTwistTauntLines);
				}
				else
				{
		...
		case 58023:
		{
			Network.Get().DisconnectFromGameServer(Network.DisconnectReason.ULDA_Dungeon);
	...
	{
		ScenarioDbId scenarioID = (ScenarioDbId)GameMgr.Get().GetMissionId();
		string playerHeroCardID = GameState.Get().GetFriendlySidePlayer().GetHero()
			.GetCardId();
		Actor playerActor = GameState.Get().GetFriendlySidePlayer().GetHero()
			.GetCard()
		...
		int enemyHealth = GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetCurrentHealth();
		if (num && GetDefeatedBossCountForFinalBoss() != 0 && enemyHealth == 300)
		{
			switch (playerHeroCardID)
			{
			case "ULDA_Reno":
