...
public class RitualActivateSpell : SuperSpell
{
	...
	public override bool AddPowerTargets()
	{
		...
		{
			Log.Spells.PrintError("RitualActivateSpell.AddPowerTargets(): Failed to get proxy ritual entity. Unable to display visuals. Proxy ritual entity ID: {0}, Proxy ritual entity tag: {1}", proxyRitualEntityId, m_ritualSpellConfig.m_proxyRitualEntityTag);
			m_willShowRitualActorVisuals = false;
		}
		if (m_taskList.IsOrigin())
		{
			if (m_ritualSpellConfig.DoesTaskListContainRitualEntity(m_taskList, proxyRitualEntityId))
			{
				...
				.GetList(), m_taskList, proxyRitualEntityId))
			{
				m_willShowRitualActorVisuals = false;
			}
		}
	...
	public override void OnSpellFinished()
	{
		...
		bool isEndOfBlock = m_taskList != null && m_taskList.IsEndOfBlock();
		if (!(num && isEndOfBlock))
		{
