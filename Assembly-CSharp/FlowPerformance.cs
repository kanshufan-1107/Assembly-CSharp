...
public class FlowPerformance
{
	...
	public void Resume()
	{
		...
		{
			float pauseDuration = m_timeProvider.TimeSinceStartup - m_pauseStartTime;
			IsPaused = false;
	...
	public void Stop()
	{
		...
		float durationInSeconds = CalculateTotalDuration();
		m_telemetryClient.SendFlowPerformance(m_id.ToString(), FlowType, m_averageFps, durationInSeconds, 20f, m_totalTriggeredThreshold, m_totalTimeUnderThreshold, m_averageTimeBelowThreshold, m_maxTimeBelowThreshold);
		OnStop();
	}
	...
	{
		bool fpsBelowThreshold = currentFps <= 20f;
		if (fpsBelowThreshold && !m_thresholdActive)
		{
			m_thresholdActive = true;
			m_thresholdStartTime = m_timeProvider.TimeSinceStartup;
			m_totalTriggeredThreshold++;
		}
		else if (!fpsBelowThreshold && m_thresholdActive)
		{
			CloseThresholdPeriod();
		}
	}
	...
	private void CloseThresholdPeriod()
	{
		...
		{
			m_thresholdActive = false;
			float totalTimeThreshold = m_timeProvider.TimeSinceStartup - m_thresholdStartTime;
			m_totalTimeUnderThreshold += totalTimeThreshold;
			m_averageTimeBelowThreshold += (totalTimeThreshold - m_averageTimeBelowThreshold) / (float)m_totalTriggeredThreshold;
			m_maxTimeBelowThreshold = Mathf.Max(m_maxTimeBelowThreshold, totalTimeThreshold);
		}
	}
