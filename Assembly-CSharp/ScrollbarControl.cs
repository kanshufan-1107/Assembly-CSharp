...
public class ScrollbarControl : MonoBehaviour
{
	...
	private void UpdateDrag()
	{
		if (m_dragging)
		{
			if (UniversalInputManager.Get().GetInputHitInfo(1 << m_DragCollider.gameObject.layer, out var hit) && hit.collider == m_DragCollider)
			{
				float left = m_LeftBone.position.x;
				float width = m_RightBone.position.x - left;
				m_thumbUnitPos = Mathf.Clamp01((hit.point.x - left) / width);
				UpdateThumb();
				HandleThumbUpdate();
			}
			else
	...
	{
		float prevPos = m_prevThumbUnitPos;
		m_prevThumbUnitPos = m_thumbUnitPos;
		if (!Mathf.Approximately(m_thumbUnitPos, prevPos))
		{
			FireUpdateEvent();
		}
	}
