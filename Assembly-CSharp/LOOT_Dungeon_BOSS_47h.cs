...
public class LOOT_Dungeon_BOSS_47h : LOOT_Dungeon
{
	...
	{
		base.PreloadAssets();
		foreach (string soundFile in new List<string> { LOOTA_BOSS_47h_Lava_FilledRoom_Death, LOOTA_BOSS_47h_Lava_FilledRoom_Emote, LOOTA_BOSS_47h_Lava_FilledRoom_Intro })
		{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(LOOTA_BOSS_47h_Lava_FilledRoom_Intro, Notification.SpeechBubbleDirection.None, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(LOOTA_BOSS_47h_Lava_FilledRoom_Emote, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
