...
public class DragCardSoundEffects : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else
		{
			...
			Vector3 positionDelta = currentPosition - m_PreviousPosition;
			m_AirVolume = Mathf.SmoothDamp(m_AirVolume, Mathf.Log(positionDelta.magnitude * 0.5f + 0.92f), ref m_AirVelocity, 0.040000003f, 1f);
	...
	{
		SoundManager.LoadedCallback airSoundCallback = delegate(AudioSource source, object userData)
		{
			if (!(source == null))
			{
		...
		SoundManager.Get().LoadAndPlay("card_motion_loop_air.prefab:d0cf08f88e847ba40ba92d3e4cc45ce5", base.gameObject, 0f, airSoundCallback);
	}

	private void LoadMagicalSound()
	{
		SoundManager.LoadedCallback magicalSoundCallback = delegate(AudioSource source, object userData)
		{
			if (!(source == null))
			{
		...
		SoundManager.Get().LoadAndPlay("card_motion_loop_magical.prefab:b954a7ca45bf25a49970664b7dd90c55", base.gameObject, 0f, magicalSoundCallback);
	}
}
