...
public class LOE_DeckTakeEvent : MonoBehaviour
{
	...
	public IEnumerator PlayTakeDeckAnim()
	{
		...
		float animDuration = m_takeDeckAnimator.GetCurrentAnimatorStateInfo(0).length;
	...
	public IEnumerator PlayReplacementDeckAnim()
	{
		...
		yield return new WaitForEndOfFrame();
		float animDuration = m_replacementDeckAnimator.GetCurrentAnimatorStateInfo(0).length;
		yield return new WaitForSeconds(animDuration);
		m_animIsPlaying = false;
	}
