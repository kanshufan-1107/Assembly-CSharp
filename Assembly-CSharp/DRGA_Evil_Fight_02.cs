...
public class DRGA_Evil_Fight_02 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_BossStartHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 105:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_09h_Female_Troll_Evil_Fight_02_Story_01_01);
			}
			break;
		case 106:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_09h_Female_Troll_Evil_Fight_02_Story_02_01);
		...
		case 107:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_09h_Female_Troll_Evil_Fight_02_Story_03_01);
			}
			break;
		case 113:
		...
		case 115:
			...
			{
				GameState.Get().SetBusy(busy: true);
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_09h_Female_Troll_Evil_Fight_02_Story_04_01);
				GameState.Get().SetBusy(busy: false);
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "DRGA_BOSS_09t"))
		{
			...
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_PlayerHeroic_FrostLichJaina_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_Puppeteering_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ICC_289"))
		{
			if (cardID == "ICC_090")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_Boss_SnowfuryGiant_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_25h_Female_Elemental_Evil_Fight_02_Boss_Moorabi_01);
		}
	}
}
