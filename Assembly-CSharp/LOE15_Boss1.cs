...
public class LOE15_Boss1 : LOE_MissionEntity
{
	...
	public override IEnumerator DoActionsAfterIntroBeforeMulligan()
	{
		...
		GameObject go = AssetLoader.Get().InstantiatePrefab("LOE_DeckTakeEvent.prefab:1d55e39305085094cbe8598e5fde37aa");
	...
	protected override IEnumerator HandleStartOfTurnWithTiming(int turn)
	{
		...
		Player enemyPlayer = GameState.Get().GetOpposingSidePlayer();
		Actor enemyActor = enemyPlayer.GetHeroCard().GetActor();
		if (!m_lowHealth && enemyPlayer.GetHero().GetCurrentHealth() < 10 && GameState.Get().GetFriendlySidePlayer().GetHero()
			.GetCurrentHealth() > 19)
		{
			m_lowHealth = true;
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechOnce("VO_LOEA15_1_LOW_HEALTH_10.prefab:a948e1a494daa01429f9c8af98ba2ba7", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			yield break;
		}
		switch (turn)
		{
		case 1:
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechOnce("VO_LOEA15_GOLDEN.prefab:c7173172b70c5ca40b1f3157ee3e5279", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechOnce("VO_LOEA15_1_TURN1_08.prefab:6a6f93dab22e2aa44bafd03d5da8597a", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			break;
		case 5:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		Actor enemyActor = GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetCard()
			.GetActor();
		if (entity.GetCardId() == "CS2_118" && !m_magmaRagerLinePlayed)
		{
			m_magmaRagerLinePlayed = true;
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechOnce("VO_LOEA15_1_MAGMA_RAGER_09.prefab:86f34787a5e7d5441a6331443d36880b", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
