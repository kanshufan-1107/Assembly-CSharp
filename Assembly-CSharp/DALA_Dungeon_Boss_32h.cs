...
public class DALA_Dungeon_Boss_32h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_32h_Female_Goblin_IntroEudora_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_Rakanishu")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_32h_Female_Goblin_IntroRakanishu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID != "DALA_Squeamlish")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_32h_Female_Goblin_Misc_01);
			break;
		case 102:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_32h_Female_Goblin_HeroPowerTwoheads_01);
			break;
		case 103:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerFade);
			break;
		case 104:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerMohawk);
			break;
		case 105:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerPompadour);
			break;
		case 106:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerQuiff);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			if (cardID == "GVG_023")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_32h_Female_Goblin_BossAutoBarber_01);
			}
		}
	}
}
