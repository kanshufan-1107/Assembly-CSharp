...
public class ScenarioDbfRecord : DbfRecord
{
	...
	public List<ClassExclusionsDbfRecord> ClassExclusions
	{
		...
		{
			int id = base.ID;
			List<ClassExclusionsDbfRecord> returnRecords = new List<ClassExclusionsDbfRecord>();
			List<ClassExclusionsDbfRecord> records = GameDbf.ClassExclusions.GetRecords();
			int i = 0;
			for (int iMax = records.Count; i < iMax; i++)
			{
				ClassExclusionsDbfRecord record = records[i];
				if (record.ScenarioId == id)
				{
	...
	public List<DeckTemplateChoicesDbfRecord> DeckTemplateChoices
	{
		get
		{
			int id = base.ID;
			List<DeckTemplateChoicesDbfRecord> returnRecords = new List<DeckTemplateChoicesDbfRecord>();
			List<DeckTemplateChoicesDbfRecord> records = GameDbf.DeckTemplateChoices.GetRecords();
			int i = 0;
			for (int iMax = records.Count; i < iMax; i++)
			{
				DeckTemplateChoicesDbfRecord record = records[i];
				if (record.ScenarioId == id)
				{
					returnRecords.Add(record);
				}
	...
	public override void SetVar(string name, object val)
	{
		...
		case "BOARD_LAYOUT":
			if (val == null)
			{
				m_boardLayout = Scenario.BoardLayout.STANDARD;
			}
			else if (val is Scenario.BoardLayout || val is int)
			{
				m_boardLayout = (Scenario.BoardLayout)val;
			}
			else if (val is string)
			{
				m_boardLayout = Scenario.ParseBoardLayoutValue((string)val);
			}
			break;
		case "ADVENTURE_ID":
	...
	public override Type GetVarType(string name)
	{
		return name switch
		{
			...
			"BOARD_LAYOUT" => typeof(Scenario.BoardLayout), 
	...
	{
		ScenarioDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(ScenarioDbfAsset)) as ScenarioDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
