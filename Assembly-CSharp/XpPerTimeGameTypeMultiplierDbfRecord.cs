...
public class XpPerTimeGameTypeMultiplierDbfRecord : DbfRecord
{
	...
	{
		XpPerTimeGameTypeMultiplierDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(XpPerTimeGameTypeMultiplierDbfAsset)) as XpPerTimeGameTypeMultiplierDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
			Debug.LogError($"XpPerTimeGameTypeMultiplierDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {resourcePath}");
			return false;
		}
