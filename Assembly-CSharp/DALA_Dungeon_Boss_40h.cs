...
public class DALA_Dungeon_Boss_40h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_40h_Female_NightElf_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_Tekahn")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_40h_Female_NightElf_IntroTekahn_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerTrigger);
			break;
		case 102:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_PlayerDemon);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "CS2_062"))
		{
			if (cardID == "EX1_308")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_40h_Female_NightElf_BossSoulfire_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_40h_Female_NightElf_BossHellfire_01);
		}
	}
}
