...
public class DRGA_Good_Fight_12 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_07h_Male_Ethereal_Good_Fight_12_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_07h_Male_Ethereal_Good_Fight_12_BossStartHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (m_Galakrond)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_07h_Male_Ethereal_Good_Fight_12_EmoteResponse_Gala_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (!m_Galakrond)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_07h_Male_Ethereal_Good_Fight_12_EmoteResponse_NoGala_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
