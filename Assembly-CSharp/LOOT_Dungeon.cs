...
public class LOOT_Dungeon : LOOT_MissionEntity
{
	...
	{
		List<GameSaveDataManager.SubkeySaveRequest> gameSaveDataRequests = new List<GameSaveDataManager.SubkeySaveRequest>();
		foreach (GameSaveKeySubkeyId subkey in m_inGameSubkeysToSave)
		{
			gameSaveDataRequests.Add(new GameSaveDataManager.SubkeySaveRequest(m_gameSaveDataClientKey, subkey, 1L));
		}
		if (gameSaveDataRequests.Count > 0)
		{
			GameSaveDataManager.Get().SaveSubkeys(gameSaveDataRequests);
		}
	...
	public override void NotifyOfGameOver(TAG_PLAYSTATE gameResult)
	{
		...
		{
			if (voLine == "VO_LOOTA_BOSS_51h_Male_Dwarf_Death_01.prefab:e5c8b619095374542bac028ed3654007")
			{
	...
	private Actor GetEnemyLoyalSidekickActor()
	{
			...
			.GetCards())
		{
			...
			if (cardEntity.GetTag(GAME_TAG.TAG_SCRIPT_DATA_NUM_2) == 1000 && cardEntity.GetCardId() == "LOOTA_829")
			{
