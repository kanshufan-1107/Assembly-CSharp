...
public class ICC_08_Finale : ICC_MissionEntity
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		Actor enemyActor = opposingPlayer.GetHeroCard().GetActor();
		Entity friendlyHero = GameState.Get().GetFriendlySidePlayer().GetHero();
		if (emoteType == EmoteType.START && friendlyHero.GetCardId() == "HERO_04b")
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_ICC08_LichKing_Male_Human_YoungArthas_02.prefab:851d35d99b9a08a4da37334369d85e29", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (opposingPlayer.GetTag(GAME_TAG.OVERRIDE_EMOTE_0) == PRIEST_GREETINGS_DISABLED_EMOTE_OVERRIDE_ID)
		{
			if (m_mutedResponses.m_triggers.Contains(emoteType))
			{
				PlayNextEmoteResponse(m_mutedResponses, enemyActor);
	...
	{
		string randomLichKingEmote = m_LichKingIntroLines[UnityEngine.Random.Range(0, m_LichKingIntroLines.Count)];
		...
		m_emoteResponseGroups = new List<EmoteResponseGroup>
		{
			...
			new EmoteResponseGroup
			{
				...
				m_responses = new List<EmoteResponse>
				{
					new EmoteResponse
					{
						m_soundName = randomLichKingEmote,
						m_stringTag = new AssetReference(randomLichKingEmote).GetLegacyAssetName()
					}
				}
			},
			new EmoteResponseGroup
	...
	protected override IEnumerator HandleStartOfTurnWithTiming(int turn)
	{
		...
		if (turn == 6)
		{
			string randomIntroEmote = m_LichKingRandomTurnLines[UnityEngine.Random.Range(0, m_LichKingRandomTurnLines.Count)];
			GameState.Get().SetBusy(busy: true);
			if (randomIntroEmote == "VO_ICC08_LichKing_Male_Human_Turn6_07")
			{
			...
			{
				yield return PlayLineOnlyOnce(enemyActor, randomIntroEmote);
			}
	...
	protected override IEnumerator HandleGameOverWithTiming(TAG_PLAYSTATE gameResult)
	{
		if (gameResult == TAG_PLAYSTATE.LOST)
		{
			...
			string voLine = "VO_ICC08_LichKing_Male_Human_Lose_03.prefab:41cdbee4c68e2a84b96710a94b454ea1";
	...
	{
		Player opposingPlayer = GameState.Get().GetOpposingSidePlayer();
		List<Actor> trappedSoulActors = new List<Actor>();
		foreach (Card card in opposingPlayer.GetBattlefieldZone().GetCards())
		{
			Entity cardEntity = card.GetEntity();
			if (cardEntity.GetControllerId() == opposingPlayer.GetPlayerId() && cardEntity.GetCardId() == "ICCA08_033")
			{
				trappedSoulActors.Add(cardEntity.GetCard().GetActor());
			}
		}
		return trappedSoulActors;
	}

	private bool isMissingeLastHorseMen()
	{
		List<string> fourHorsemen = new List<string> { "ICC_829t5", "ICC_829t2", "ICC_829t3", "ICC_829t4" };
		Player player = GameState.Get().GetOpposingSidePlayer();
		foreach (Card card in player.GetBattlefieldZone().GetCards())
		{
			Entity cardEntity = card.GetEntity();
			if (cardEntity.GetControllerId() == player.GetPlayerId() && fourHorsemen.Contains(cardEntity.GetCardId()))
			{
				fourHorsemen.Remove(cardEntity.GetCardId());
			}
		}
		if (fourHorsemen.Count == 1)
		{
			return true;
		}
		return false;
	...
	{
		TAG_CLASS friendlyHeroClass = GameState.Get().GetFriendlySidePlayer().GetStartingHero()
			.GetClass();
		if (m_HeroVictoryLines.TryGetValue(friendlyHeroClass, out var voLines))
		{
