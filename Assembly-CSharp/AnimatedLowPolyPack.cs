...
public class AnimatedLowPolyPack : MonoBehaviour
{
	...
	private Vector3 m_originalAmountBannerPos = Vector3.zero;
	...
	private void Awake()
	{
		...
		{
			m_originalAmountBannerPos = m_AmountBanner.transform.localPosition;
		}
	}
	...
	public bool FlyOut(float animTime, float delay)
	{
		...
		base.transform.localEulerAngles = m_flyOutLocalAngles;
		Hashtable args = iTweenManager.Get().GetTweenHashTable();
		args.Add("position", m_targetOffScreenLocalPos);
		args.Add("islocal", true);
		args.Add("time", animTime);
		args.Add("delay", delay);
		args.Add("easetype", iTween.EaseType.linear);
		args.Add("oncomplete", "OnHidden");
		args.Add("oncompletetarget", base.gameObject);
		iTween.Stop(base.gameObject);
		iTween.MoveTo(base.gameObject, args);
	...
	public void ResetBannerPosition()
	{
		if (m_AmountBanner != null)
		{
			m_AmountBanner.transform.localPosition = m_originalAmountBannerPos;
		}
	}

	public void UpdateBannerPosition(float zOffset)
	{
		if (m_AmountBanner != null)
		{
			m_AmountBanner.transform.localPosition = new Vector3(m_AmountBanner.transform.localPosition.x, m_AmountBanner.transform.localPosition.y, zOffset);
		}
	}
