...
public class KAR04_Julianne : KAR_MissionEntity
{
	...
	private Actor GetRomulo()
	{
		...
		foreach (Card card in player.GetBattlefieldZone().GetCards())
		{
			...
			if (cardEntity.GetControllerId() == player.GetPlayerId() && (cardEntity.GetCardId() == "KARA_06_01" || cardEntity.GetCardId() == "KARA_06_01heroic"))
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
			GameState.Get().SetBusy(busy: true);
			yield return PlayBossLine(enemyActor, "VO_Julianne_Female_Human_JulianneHeroPower_01.prefab:52cf3ed754f5ae647a1fb2a27ae8e37d");
			GameState.Get().SetBusy(busy: false);
		}
	}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "CS2_074"))
		{
			...
			{
				GameState.Get().SetBusy(busy: true);
				yield return PlayEasterEggLine(enemyActor, "VO_Julianne_Female_Human_JulianneFeignDeath_03.prefab:e28f8db1c88f12f4bba962b65e9ed936");
				GameState.Get().SetBusy(busy: false);
			}
			yield break;
		}
