...
public class ArenaMedal : PegUIElement
{
	...
	{
		bool isQuestLog = SceneMgr.Get().IsModeRequested(SceneMgr.Mode.HUB);
		string headerText;
		string bodyText;
		if (Options.Get().GetBool(Option.IN_RANKED_PLAY_MODE) || isQuestLog)
		{
			headerText = GetMedalName();
			bodyText = GameStrings.Format("GLOBAL_MEDAL_ARENA_TOOLTIP_BODY", m_medal);
		}
		else
		{
			headerText = GameStrings.Get("GLUE_TOURNAMENT_UNRANKED_MODE");
			bodyText = GameStrings.Get("GLUE_TOURNAMENT_UNRANKED_DESC");
		}
		base.gameObject.GetComponent<TooltipZone>().ShowLayerTooltip(headerText, bodyText);
	}
	...
	{
		string key = "GLOBAL_ARENA_MEDAL_" + (m_medal + 1);
