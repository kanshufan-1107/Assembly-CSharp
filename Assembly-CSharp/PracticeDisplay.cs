...
public class PracticeDisplay : MonoBehaviour
{
	...
	{
		s_instance = this;
		GameObject tray = (GameObject)GameUtils.Instantiate(m_practicePickerTrayPrefab, m_practicePickerTrayContainer);
		m_practicePickerTray = tray.GetComponent<PracticePickerTrayDisplay>();
		if ((bool)UniversalInputManager.UsePhoneUI)
		{
			LayerUtils.SetLayer(m_practicePickerTray, GameLayer.IgnoreFullScreenEffects);
		}
	...
	public void DismissTrays()
	{
		if (m_practicePickerTray != null)
		{
	...
	private void InitializeTrays()
	{
		...
		string name = GameUtils.GetAdventureDataRecord((int)selectedAdventure, modeDbId).Name;
		m_deckPickerTray.SetHeaderText(name);
