...
public class AdventureWingOpenBanner : MonoBehaviour
{
	...
	public void ShowBanner(OnBannerHidden onBannerHiddenCallback = null)
	{
		...
		{
			string gameString = new AssetReference(m_VOQuoteLine).GetLegacyAssetName();
			NotificationManager.Get().CreateCharacterQuote(m_VOQuotePrefab, m_VOQuotePosition, GameStrings.Get(gameString), m_VOQuoteLine, allowRepeatDuringSession: true, 0f, null, CanvasAnchor.CENTER);
		}
		ScreenEffectParameters screenEffectParameters = ScreenEffectParameters.BlurVignetteDesaturatePerspective;
		screenEffectParameters.Time = m_showTime;
	...
	public void HideBanner()
	{
		...
		{
			string gameString = new AssetReference(m_BannerDismissedQuoteLine).GetLegacyAssetName();
			NotificationManager.Get().CreateCharacterQuote(m_BannerDismissedQuotePrefab, m_BannerDismissedQuotePosition, GameStrings.Get(gameString), m_BannerDismissedQuoteLine, allowRepeatDuringSession: true, 0f, null, CanvasAnchor.CENTER);
		}
	}
}
