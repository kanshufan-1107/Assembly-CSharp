...
public class DALA_Dungeon_Boss_18h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_18h_Female_Draenei_IntroOlBarkeye_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_Vessina")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_18h_Female_Draenei_IntroVessina_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "EX1_244"))
		{
			if (cardID == "LOOT_518")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_18h_Female_Draenei_BossWindshearStormcaller_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_18h_Female_Draenei_BossTotemicMight_01);
		}
	}
}
