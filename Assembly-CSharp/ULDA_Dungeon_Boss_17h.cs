...
public class ULDA_Dungeon_Boss_17h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			if (playerHeroCardID != "ULDA_Elise" && playerHeroCardID != "ULDA_Finley")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (missionEvent == 101)
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_17h_Male_Gnome_BossCheatDeathTriggerLeperGnome_01);
		}
		else
		{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULDA_115"))
		{
			if (cardID == "ULD_167")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_17h_Male_Gnome_PlayerTrigger_Diseased_Vulture_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_17h_Male_Gnome_PlayerTrigger_Gnomebliterator_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "BOT_286"))
		{
			if (cardID == "ICC_809")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_17h_Male_Gnome_BossPlagueScientist_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_17h_Male_Gnome_BossNecriumBlade_01);
		}
	}
}
