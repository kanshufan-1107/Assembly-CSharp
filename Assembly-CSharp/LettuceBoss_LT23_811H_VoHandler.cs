...
public class LettuceBoss_LT23_811H_VoHandler : VoPlaybackHandler
{
	...
	private List<string> m_IdleLines = new List<string> { VO_Nzoth_Male_OldGod_LETL_Idle_01 };

	public override void PreloadAssets()
	{
		base.PreloadAssets();
		List<string> soundFiles = new List<string> { VO_Nzoth_Male_OldGod_LETL_Intro_02, VO_Nzoth_Male_OldGod_LETL_Attack_02, VO_Nzoth_Male_OldGod_LETL_Attack_03, VO_Nzoth_Male_OldGod_LETL_Idle_01, VO_Nzoth_Male_OldGod_LETL_Death_02 };
	...
	{
		base.OnCreateGame();
		m_introLine = VO_Nzoth_Male_OldGod_LETL_Intro_02;
		m_deathLine = VO_Nzoth_Male_OldGod_LETL_Death_02;
	}
	...
	public override IEnumerator RespondToWillPlayCardWithTiming(string cardID, Entity playedEntity)
	{
		...
		Actor bossActor = FindEnemyActorInPlayByDesignCode("LT23_811H");
		if (!(playedEntity.GetLettuceAbilityOwner().GetCardId() == "LT23_811H"))
		{
			yield break;
		}
		if (!(cardID == "LT23_811P1"))
		{
			if (cardID == "LT23_811P2")
			{
				GameState.Get().SetBusy(busy: true);
				yield return MissionPlayVOOnce(bossActor, VO_Nzoth_Male_OldGod_LETL_Attack_03);
		...
		{
			GameState.Get().SetBusy(busy: true);
			yield return MissionPlayVOOnce(bossActor, VO_Nzoth_Male_OldGod_LETL_Attack_02);
			GameState.Get().SetBusy(busy: false);
		}
	}
