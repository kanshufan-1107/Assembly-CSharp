...
public class MagneticPlaySpell : Spell
{
	...
	public override bool AttachPowerTaskList(PowerTaskList taskList)
	{
		...
		{
			if (task.GetPower() is Network.HistMetaData { MetaType: HistoryMeta.Type.TARGET })
			{
				return true;
			}
		}
	...
	private IEnumerator DoMagneticEffect()
	{
		...
		if (magneticPlayData == null)
		{
			ZonePlay playZone = sourceCard.GetController().GetBattlefieldZone();
			magneticPlayData = new MagneticPlayData
			{
				m_playedCard = sourceCard,
				m_targetMech = targetCard,
				m_beamSpell = (MagneticBeamSpell)SpellManager.Get().GetSpell(playZone.GetMagneticBeamSpell())
			};
			sourceCard.SetMagneticPlayData(magneticPlayData);
			targetCard.SetIsMagneticTarget(isTarget: true);
		...
		target.SetIsMagneticTarget(isTarget: false);
		OnStateFinished();
		OnSpellFinished();
	}
}
