...
public class BouncingBlade : SuperSpell
{
	...
	protected override void OnAction(SpellStateType prevStateType)
	{
		...
		m_isDone = false;
		bool handlingLastTaskList = IsHandlingLastTaskList();
		for (int i = 0; i < m_targets.Count; i++)
		{
			GameObject visualTarget = m_targets[i];
			int metaDataIdx = GetMetaDataIndexForTarget(i);
			Target target = new Target();
			target.VisualTarget = visualTarget;
			target.TargetPosition = visualTarget.transform.position;
			target.MetaDataIdx = metaDataIdx;
			target.isMinion = true;
			if (i == m_targets.Count - 1)
			{
				target.LastTarget = true;
			}
			if (handlingLastTaskList)
			{
			...
			if (!target.LastTarget)
			{
				...
				randomTarget.TargetPosition = AcquireRandomBoardTarget(out randomTarget.Offscreen);
				randomTarget.isMinion = false;
				randomTarget.LastTarget = false;
				m_TargetQueue.Add(randomTarget);
			}
		}
	}
	...
	private IEnumerator BladeRunner()
	{
		while (!m_isDone)
		{
			...
			randomTarget.isMinion = false;
			randomTarget.LastTarget = false;
			AnimateToNextTarget(randomTarget);
			while (m_Animating)
			{
				yield return null;
			}
	...
	{
		Vector3 boardCenter = Board.Get().FindBone("CenterPointBone").transform.position;
		Vector3 orgLocPos = m_HitBonesRoot.transform.localPosition;
		m_HitBonesRoot.transform.position = boardCenter;
		foreach (HitBonesType hitBone in m_HitBones)
		{
			hitBone.SetPosition(hitBone.Bone.transform.position);
		}
		m_HitBonesRoot.transform.localPosition = orgLocPos;
	}
	...
	private void RampBladeVolume()
	{
		...
		Action<object> bladeSpinningVolUpdate = delegate(object amount)
		{
			SoundManager.Get().SetVolume(m_BladeSpinning, (float)amount);
		};
		iTween.ValueTo(m_BladeSpinning.gameObject, iTween.Hash("name", "BladeSpinningSound", "from", m_BladeSpinningMinVol, "to", m_BladeSpinningMaxVol, "time", m_BladeSpinningRampTime, "easetype", iTween.EaseType.linear, "onupdate", bladeSpinningVolUpdate, "onupdatetarget", m_BladeSpinning.gameObject));
	}
	...
	private void AnimationComplete(Target target)
	{
		...
		AudioSource impactSound = (target.isMinion ? m_BladeHitMinion : ((!target.Offscreen) ? m_BladeHitBoardCorner : m_BladeHitOffScreen));
		if (impactSound != null)
		{
			impactSound.gameObject.transform.position = target.TargetPosition;
			SoundManager.Get().Play(impactSound);
		}
	}
	...
	private Vector3 AcquireRandomBoardTarget(out bool offscreen)
	{
		...
		List<HitBonesType> possibleTargets = new List<HitBonesType>();
		if (offscreen)
		{
			foreach (HitBonesType hitBone in m_HitBones)
			{
		...
		else
		{
			foreach (HitBonesType hitBone2 in m_HitBones)
			{
				...
				{
					possibleTargets.Add(hitBone2);
				}
			}
		}
		int rndIdx = UnityEngine.Random.Range(0, possibleTargets.Count);
		m_PreviousHitBone = possibleTargets[rndIdx];
		return possibleTargets[rndIdx].GetPosition();
	}
}
