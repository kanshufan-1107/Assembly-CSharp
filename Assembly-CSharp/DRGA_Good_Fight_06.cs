...
public class DRGA_Good_Fight_06 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_EmoteResponseHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 100:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_17h_Female_Dragon_Good_Fight_06_Backstory_01a_01);
		...
		case 104:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_17h_Female_Dragon_Good_Fight_06_Misc_01_01);
			}
			break;
		case 105:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_17h_Female_Dragon_Good_Fight_06_Misc_02_01);
			}
			break;
		case 106:
			if (!m_Heroic)
			{
				yield return PlayLineAlways(friendlyActor, VO_DRGA_BOSS_17h_Female_Dragon_Good_Fight_06_Misc_03_01);
			}
			break;
		case 107:
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_BossAttack_01);
			break;
		default:
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "EX1_561":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Player_Alexstrasza_01);
				break;
			case "GIL_504":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Player_Hagatha_01);
				break;
			case "DAL_009":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Player_HagathasScheme_01);
				break;
			case "GIL_820":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Player_Shudderwock_01);
				break;
			}
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "DRGA_BOSS_06t"))
		{
			...
			{
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Boss_WormholeLines);
			}
		}
		else
		{
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_VO_DRGA_BOSS_06h_Female_Orc_Good_Fight_06_Boss_WailOfTheShudderwockLines);
		}
	}
}
