...
public class BOTA_Lethal_Puzzle_3 : BOTA_MissionEntity
{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (cardId == "LOOT_373")
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_BOTA_BOSS_12h_Female_WaterSpirit_PlayHealingRain_01);
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 777:
		{
			string s = GetLethalCompleteLine();
			if (s != null)
			{
				GameState.Get().SetBusy(busy: true);
				yield return PlayBossLine(enemyActor, s);
				GameState.Get().SetBusy(busy: false);
			}
			break;
		}
