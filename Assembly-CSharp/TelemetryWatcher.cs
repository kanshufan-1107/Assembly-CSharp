...
public static class TelemetryWatcher
{
	...
	{
		if (!s_watchTypeSetupActions.TryGetValue(watchType, out var setupAction))
		{
	...
	public static void StopWatchingFor(TelemetryWatcherWatchType watchType)
	{
		...
		if (s_watchTypeTeardownActions.TryGetValue(watchType, out var teardownAction))
		{
			teardownAction();
		}
	...
	private static void OnBoxButtonPressStoreWatcher(Box.ButtonType type, bool isShowingTutorialPreview, object userData)
	{
	...
	private static void OnBoxButtonPressCollectionWatcher(Box.ButtonType type, bool isShowingTutorialPreview, object userData)
	{
