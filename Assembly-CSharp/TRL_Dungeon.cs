...
public class TRL_Dungeon : TRL_MissionEntity
{
	...
	{
		base.PreloadAssets();
		foreach (string soundFile in new List<string> { VO_TRLA_209h_Male_Troll_Shrine_Killed_01, VO_TRLA_209h_Male_Troll_Shrine_Killed_02, VO_TRLA_209h_Male_Troll_Shrine_Killed_03, VO_TRLA_209h_Male_Troll_Shrine_Killed_04 })
		{
	...
	public override void NotifyOfGameOver(TAG_PLAYSTATE gameResult)
	{
		...
		if (!m_enemySpeaking && !string.IsNullOrEmpty(s_deathLine) && gameResult == TAG_PLAYSTATE.WON)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_deathLine, Notification.SpeechBubbleDirection.None, enemyActor));
			}
			else
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_deathLine, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		case EmoteType.GREETINGS:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineGreeting, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.WOW:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineWow, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.THREATEN:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineThreaten, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.WELL_PLAYED:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineWellPlayed, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.THANKS:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineThanks, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.OOPS:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineOops, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		case EmoteType.SORRY:
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(s_responseLineSorry, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			break;
		}
	}
	...
	private IEnumerator PlayRandomRikkarShrineDeathLine()
	{
		Actor playerActor = GameState.Get().GetFriendlySidePlayer().GetHeroCard()
			.GetActor();
		if (!GameState.Get().IsFriendlySidePlayerTurn() && CanPlayVOLines(playerActor.GetEntity(), VOSpeaker.FRIENDLY_HERO))
		{
			string randomLine = PopRandomLineWithChance(m_RikkarRandomLines);
			if (randomLine != null)
			{
				yield return PlayLineOnlyOnce(playerActor, randomLine);
			}
		}
	}
	...
	private IEnumerator PlayClassOrGenericShrineDeathLine(List<string> classDeathLines)
	{
		Actor enemyActor = GameState.Get().GetOpposingSidePlayer().GetHeroCard()
			.GetActor();
		string randomLine = PopRandomLineWithChance(classDeathLines);
		if (randomLine != null)
		{
			yield return PlayLineOnlyOnce(enemyActor, randomLine);
			yield break;
		}
		randomLine = PopRandomLineWithChance(s_genericShrineDeathLines);
		if (randomLine != null)
		{
			yield return PlayLineOnlyOnce(enemyActor, randomLine);
		}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 202:
		{
			string randomLine = PopRandomLineWithChance(s_bossShrineDeathLines);
			if (randomLine != null)
			{
				yield return PlayBossLine(enemyActor, randomLine);
			}
			break;
		}
		default:
