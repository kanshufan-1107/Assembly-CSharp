...
public class DALA_Dungeon_Boss_14h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_14h_Female_KulTiran_IntroOlBarkeye_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_George")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_14h_Female_KulTiran_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "EX1_407":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossBrawl);
				break;
			case "UNG_929":
			case "LOOT_367":
			case "EX1_604":
			case "BRM_019":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossMinions);
				break;
			}
		}
	}
