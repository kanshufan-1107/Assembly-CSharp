...
public class ReconnectHelperDialog : DialogBase
{
	...
	private void Start()
	{
		...
		ReconnectMgr.Get().OnUtilReconnectComplete += OnUtilReconnectComplete;
	}
	...
	protected override void OnDestroy()
	{
		...
		{
			reconnectMgr.OnUtilReconnectComplete -= OnUtilReconnectComplete;
		}
	}
	...
	{
		Layout stateLayout = m_stateLayouts[m_state];
		m_continueButton.SetText(stateLayout.m_continueButtonText);
		m_choiceOneButton.SetText(stateLayout.m_choiceOneButtonText);
		m_choiceTwoButton.SetText(stateLayout.m_choiceTwoButtonText);
		m_continueButtonContainer.SetActive(!stateLayout.m_twoButtons);
		m_choiceButtonContainer.SetActive(stateLayout.m_twoButtons);
		m_successRingContainer.SetActive(stateLayout.m_successRingState != SpellStateType.NONE);
		if (stateLayout.m_successRingState != 0)
		{
			m_successRingSpell.ActivateState(stateLayout.m_successRingState);
		}
		for (int i = 0; i < m_panels.Count; i++)
		{
			GameObject obj = m_panels[i];
			obj.SetActive(obj == stateLayout.m_activePanel);
		}
	...
	private void OnUtilReconnectComplete()
	{
		if (m_state == DialogState.IN_PROGRESS)
		{
			OnReconnectSuccess();
