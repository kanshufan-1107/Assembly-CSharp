...
public class QuestDialogOnCompleteDbfRecord : DbfRecord
{
	...
	{
		QuestDialogOnCompleteDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(QuestDialogOnCompleteDbfAsset)) as QuestDialogOnCompleteDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
			Debug.LogError($"QuestDialogOnCompleteDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {resourcePath}");
			return false;
		}
