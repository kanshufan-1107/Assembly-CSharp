...
public class RankedRewardDisplay : MonoBehaviour
{
	...
	public void Initialize(TranslatedMedalInfo medalInfo, List<List<RewardData>> rewardDataList, Action callback)
	{
		...
		RankedPlayDataModel rankedPlayDataModel = medalInfo.CreateDataModel(RankedMedal.DisplayMode.Chest);
		m_widget.BindDataModel(rankedPlayDataModel);
		foreach (List<RewardData> rewardData2 in rewardDataList)
		{
			RewardListDataModel rewardListDataModel = new RewardListDataModel();
			foreach (RewardData rewardData in rewardData2)
			{
				if (rewardData != null)
				{
