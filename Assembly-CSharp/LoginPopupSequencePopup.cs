...
public class LoginPopupSequencePopup : BasicPopup
{
	...
	public void LoadAssetsAndShowWhenReady()
	{
		if (!string.IsNullOrEmpty(m_info.m_backgroundMaterialReference))
		{
			...
			if (!AssetLoader.Get().LoadMaterial(m_info.m_backgroundMaterialReference, OnBackgroundMaterialLoaded))
			{
				OnBackgroundMaterialLoaded(m_info.m_backgroundMaterialReference, null, null);
			}
		}
	...
	{
		CardActorLoadedData data = (CardActorLoadedData)callbackData;
		using (data.m_fullDef)
		{
			m_cardReady = true;
			m_cardActor = go.GetComponent<Actor>();
			m_cardActor.SetCardDef(data.m_fullDef.DisposableCardDef);
			m_cardActor.SetEntityDef(data.m_fullDef.EntityDef);
			m_cardActor.ContactShadow(visible: true);
			m_cardActor.SetPremium(data.m_premium);
			m_cardActor.UpdateAllComponents();
			GameUtils.SetParent(m_cardActor, m_cardBone, withRotation: true);
			LayerUtils.SetLayer(m_cardActor, m_cardBone.gameObject.layer);
		}
	}
