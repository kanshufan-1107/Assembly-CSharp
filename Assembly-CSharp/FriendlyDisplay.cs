...
public class FriendlyDisplay : MonoBehaviour
{
	...
	private void DisableOtherModeStuff()
	{
		...
		{
			Camera fullScreenEffectsCam = CameraUtils.FindFullScreenEffectsCamera(activeOnly: true);
			if (fullScreenEffectsCam != null)
			{
				fullScreenEffectsCam.GetComponent<FullScreenEffects>().Disable();
			}
		}
	}
	...
	private void InitHeroPicker()
	{
		...
		List<ScenarioGuestHeroesDbfRecord> scenarioHeroes = null;
		if (scenarioId > 0)
		{
			scenarioHeroes = GameUtils.GetScenarioGuestHeroes(scenarioId);
		}
		if (scenarioHeroes != null && scenarioHeroes.Count > 0)
		{
