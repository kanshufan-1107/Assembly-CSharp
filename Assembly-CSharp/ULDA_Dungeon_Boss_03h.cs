...
public class ULDA_Dungeon_Boss_03h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_03h_Female_Human_IntroBrannResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "ULDA_Elise")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_03h_Female_Human_IntroEliseResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_195"))
		{
			if (cardID == "ULD_163")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_03h_Female_Human_PlayerTrigger_Expired_Merchant_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_03h_Female_Human_PlayerTrigger_Frightened_Flunky_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_160"))
		{
			if (cardID == "ULD_170")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_03h_Female_Human_BossTriggerWeaponizedWasp_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_03h_Female_Human_BossTriggerSinisterDeal_01);
		}
	}
}
