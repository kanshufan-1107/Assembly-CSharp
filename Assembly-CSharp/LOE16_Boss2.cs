...
public class LOE16_Boss2 : LOE_MissionEntity
{
	...
	public override string UpdateCardText(Card card, Actor bigCardActor, string text)
	{
		...
		if (opposingPlayer.GetHeroPowerCard() != card)
		{
			return text;
		}
		int charge = opposingPlayer.GetHeroPower().GetTag(GAME_TAG.ELECTRIC_CHARGE_LEVEL);
		if (GameState.Get().GetGameEntity().GetTag(GAME_TAG.TURN) < 2)
		{
			charge = 3;
		}
		string staffChargeString = "";
		switch (charge)
		{
		case 0:
			staffChargeString = "LOEA16_2_STAFF_TEXT_CHARGE_EXPLODED";
			break;
		case 1:
			staffChargeString = "LOEA16_2_STAFF_TEXT_CHARGE_1";
			break;
		case 2:
			staffChargeString = "LOEA16_2_STAFF_TEXT_CHARGE_2";
			break;
		case 3:
			staffChargeString = "LOEA16_2_STAFF_TEXT_CHARGE_0";
			break;
		}
		return GameStrings.Get(staffChargeString);
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		case "LOEA16_5":
			if (!m_artifactLinePlayed)
			{
				...
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechOnce("VO_LOEA16_1_CARD_04.prefab:c929045c2c500af41ae4326197c58e1c", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			break;
		}
	}
