...
public class SwipeHandler : PegUICustomBehavior
{
	...
	{
		float swipeDistance = m_swipeStart.x - InputCollection.GetMousePosition().x;
		float minDistance = 0.09f + ((m_swipeElement != null) ? 0.035f : 0f);
		float minDistancePixels = (float)Screen.width * minDistance;
		if (Mathf.Abs(swipeDistance) > minDistancePixels)
		{
			SWIPE_DIRECTION direction = ((!(swipeDistance < 0f)) ? SWIPE_DIRECTION.LEFT : SWIPE_DIRECTION.RIGHT);
	...
	private bool HandleSwipeGesture()
	{
		...
		if (InputCollection.GetMouseButtonDown(0) && InSwipeRect(InputCollection.GetMousePosition()))
		{
			m_checkingPossibleSwipe = true;
			m_swipeDetectTimer = 0f;
			m_swipeStart = InputCollection.GetMousePosition();
