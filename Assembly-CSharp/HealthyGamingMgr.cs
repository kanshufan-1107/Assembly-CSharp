...
public class HealthyGamingMgr : IService, IHasUpdate
{
	...
	{
		return ServiceManager.Get<HealthyGamingMgr>();
	}
	...
	private IEnumerator InitNetworkData()
	{
		...
		if (m_AccountCountry == "CHN" || m_AccountCountry == "KOR")
		{
			if (PlatformSettings.IsMobile())
			{
				...
				{
					caisEnabled = features.CaisEnabledMobileChina;
				}
				else if (m_AccountCountry == "KOR")
				{
					caisEnabled = features.CaisEnabledMobileSouthKorea;
				}
			}
			else
			{
				caisEnabled = features.CaisEnabledNonMobile;
			}
		}
		...
		if (m_AccountCountry == "CHN")
		{
			string stringKey = "GLOBAL_HEALTHY_GAMING_CHINA_CAIS_ACTIVE";
			if ((bool)UniversalInputManager.UsePhoneUI && GameStrings.HasKey(stringKey + "_PHONE"))
			{
				stringKey += "_PHONE";
			}
			string message = GameStrings.Get(stringKey);
			SocialToastMgr.Get().AddToast(UserAttentionBlocker.ALL_EXCEPT_FATAL_ERROR_SCENE, message, SocialToastMgr.TOAST_TYPE.DEFAULT, CHINA_CAIS_ACTIVE_DISPLAY_TIME_SECONDS);
	...
	private void KoreaRestrictions()
	{
		...
		if (m_DebugMode)
		{
			Debug.LogFormat("[HealthyGaming] Minutes Played: " + seconds / 60f);
		}
		if (!(seconds < m_NextMessageDisplayTime))
		{
			m_NextMessageDisplayTime += KOREA_MESSAGE_FREQUENCY_MINUTES * 60f;
			int hours = (int)(seconds / 60f) / 60;
			SocialToastMgr.Get().AddToast(UserAttentionBlocker.ALL_EXCEPT_FATAL_ERROR_SCENE, GameStrings.Format("GLOBAL_HEALTHY_GAMING_TOAST", hours), SocialToastMgr.TOAST_TYPE.DEFAULT, KOREA_MESSAGE_DISPLAY_TIME_SECONDS);
		}
	}
	...
	private void ChinaRestrictions_LessThan3Hours(int minutesPlayed)
	{
		...
		string stringKey = "GLOBAL_HEALTHY_GAMING_CHINA_LESS_THAN_THREE_HOURS";
		if ((bool)UniversalInputManager.UsePhoneUI && GameStrings.HasKey(stringKey + "_PHONE"))
		{
			stringKey += "_PHONE";
		}
		int hoursPlayed = minutesPlayed / 60;
		string message = GameStrings.Format(stringKey, hoursPlayed);
		SocialToastMgr.Get().AddToast(UserAttentionBlocker.ALL_EXCEPT_FATAL_ERROR_SCENE, message, SocialToastMgr.TOAST_TYPE.DEFAULT, CHINA_FIRST_MESSAGE_DISPLAY_TIME_SECONDS);
		if (m_DebugMode)
		{
			...
			Debug.LogFormat("[HealthyGaming] First message: {0}", GameStrings.Format("GLOBAL_HEALTHY_GAMING_CHINA_LESS_THAN_THREE_HOURS", hoursPlayed));
			Debug.LogFormat("[HealthyGaming] NextMessageDisplayTime: " + m_NextMessageDisplayTime);
		}
		else
		{
			Debug.LogFormat("[HealthyGaming] Time: {0} sec,  Played: {1} min,  First message: {2}", Time.realtimeSinceStartup, minutesPlayed, message);
		}
	}
	...
	private void ChinaRestrictions_3to5Hours(int minutesPlayed)
	{
		...
		string stringKey = "GLOBAL_HEALTHY_GAMING_CHINA_THREE_TO_FIVE_HOURS";
		if ((bool)UniversalInputManager.UsePhoneUI && GameStrings.HasKey(stringKey + "_PHONE"))
		{
			stringKey += "_PHONE";
		}
		string message = GameStrings.Get(stringKey);
		SocialToastMgr.Get().AddToast(UserAttentionBlocker.ALL_EXCEPT_FATAL_ERROR_SCENE, message, SocialToastMgr.TOAST_TYPE.DEFAULT, CHINA_SECOND_MESSAGE_DISPLAY_TIME_SECONDS);
		if (m_DebugMode)
		{
		...
		{
			Debug.LogFormat("[HealthyGaming] Time: {0} sec,  Played: {1} min,  Second message: {2}", Time.realtimeSinceStartup, minutesPlayed, message);
		}
	}
	...
	private void ChinaRestrictions_MoreThan5Hours(int minutesPlayed)
	{
		...
		string stringKey = "GLOBAL_HEALTHY_GAMING_CHINA_MORE_THAN_FIVE_HOURS";
		if ((bool)UniversalInputManager.UsePhoneUI && GameStrings.HasKey(stringKey + "_PHONE"))
		{
			stringKey += "_PHONE";
		}
		string message = GameStrings.Get(stringKey);
		SocialToastMgr.Get().AddToast(UserAttentionBlocker.ALL_EXCEPT_FATAL_ERROR_SCENE, message, SocialToastMgr.TOAST_TYPE.DEFAULT, CHINA_THIRD_MESSAGE_DISPLAY_TIME_SECONDS);
		if (m_DebugMode)
		{
		...
		{
			Debug.LogFormat("[HealthyGaming] Time: {0} sec,  Played: {1} min,  Third message: {2}", Time.realtimeSinceStartup, minutesPlayed, message);
		}
	}
