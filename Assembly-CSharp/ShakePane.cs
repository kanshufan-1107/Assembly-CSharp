...
public class ShakePane : MonoBehaviour
{
	...
	private IEnumerator AnimateShakeyObjectCoroutine(float xRotationAmount, float translationAmount, float shakeTime, float delay)
	{
		...
		args.Add("x", xRotationAmount);
		args.Add("time", shakeTime);
		...
		iTween.PunchRotation(m_shakeyObject, args);
		if (translationAmount != 0f)
		{
			m_shakeyObject.transform.localPosition = m_shakeyObjectOriginalLocalPosition;
			Hashtable transArgs = iTweenManager.Get().GetTweenHashTable();
			transArgs.Add("y", translationAmount);
			transArgs.Add("time", shakeTime);
			transArgs.Add("delay", 0.001f);
			transArgs.Add("oncomplete", (Action<object>)delegate
			{
				m_shakeyObject.transform.localPosition = m_shakeyObjectOriginalLocalPosition;
			});
			iTween.PunchPosition(m_shakeyObject, transArgs);
		}
	}
}
