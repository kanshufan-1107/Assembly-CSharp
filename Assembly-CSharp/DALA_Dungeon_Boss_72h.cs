...
public class DALA_Dungeon_Boss_72h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
			case "DALA_George":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_72h_Male_Orc_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				break;
			case "DALA_Chu":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_72h_Male_Orc_IntroChu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				break;
			case "DALA_Tekahn":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_72h_Male_Orc_IntroTekahn_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				break;
			default:
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerTriggers);
			break;
		case 102:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_PlayerSelfDamage);
			break;
		case 103:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_72h_Male_Orc_PlayerArmor_01);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			if (cardID == "CS2_062")
			{
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_BossHellfire);
			}
		}
	}
}
