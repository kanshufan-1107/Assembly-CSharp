...
public class DeckTemplatePhoneTray : MonoBehaviour
{
	...
	private void OnCardCountUpdated(int cardCount, int maxCount)
	{
		...
		if (cardCount > 0)
		{
			...
			if (cardCount < maxCount)
			{
				...
				cardCountText = GameStrings.Format("GLUE_DECK_TRAY_COUNT", cardCount, maxCount);
			}
		}
	...
	private void ShowDeckBigCard(DeckTrayDeckTileVisual cardTile, float delay = 0f)
	{
		...
		GhostCard.Type ghostType = GhostCard.GetGhostTypeFromSlot(m_cardsContent.GetEditingDeck(), cardTile.GetSlot());
