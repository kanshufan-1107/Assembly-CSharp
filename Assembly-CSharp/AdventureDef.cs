...
public class AdventureDef : MonoBehaviour
{
	...
	public void Init(AdventureDbfRecord advRecord, List<AdventureDataDbfRecord> advDataRecords)
	{
		...
		foreach (AdventureDataDbfRecord advDataRecord in advDataRecords)
		{
			...
			string subDefAssetPath = advDataRecord.AdventureSubDefPrefab;
			if (string.IsNullOrEmpty(subDefAssetPath))
			{
				continue;
			}
			GameObject subDefObj = AssetLoader.Get().InstantiatePrefab(subDefAssetPath);
			if (!(subDefObj == null))
			{
				AdventureSubDef subDef = subDefObj.GetComponent<AdventureSubDef>();
				if (subDef == null)
				{
					Debug.LogError($"{subDefAssetPath} object does not contain AdventureSubDef component.");
					UnityEngine.Object.Destroy(subDefObj);
				}
				else
				{
					subDef.Init(advDataRecord);
					m_SubDefs.Add(subDef.GetAdventureModeId(), subDef);
				}
			}
		}
	}
	...
	{
		AdventureSubDef subDef = null;
		m_SubDefs.TryGetValue(modeId, out subDef);
		return subDef;
	}
