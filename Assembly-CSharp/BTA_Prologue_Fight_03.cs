...
public class BTA_Prologue_Fight_03 : BTA_Prologue_Dungeon
{
	...
	protected override void OnBossHeroPowerPlayed(Entity entity)
	{
		...
		if (!(chosenLine == ""))
		{
			if (chosenLine == VO_Prologue_Mannoroth_Male_Demon_Prologue_Mission3_Loss_01)
			{
				PlaySound(chosenLine);
			}
			else
			{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_Prologue_Mannoroth_Male_Demon_Prologue_Mannoroth_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 605:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(enemyActor, VO_Prologue_Mannoroth_Male_Demon_Prologue_Mission3_ExchangeC_01_01);
			yield return PlayLineAlways(friendlyActor, VO_TB_Hero_Illidan2_Male_NightElf_Prologue_Mission3_ExchangeC_02_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 606:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(enemyActor, VO_Prologue_Mannoroth_Male_Demon_Prologue_Mission3_ExchangeB_01_01);
			yield return PlayLineAlways(friendlyActor, VO_TB_Hero_Illidan2_Male_NightElf_Prologue_Mission3_ExchangeB_02_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 607:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(enemyActor, VO_Prologue_Mannoroth_Male_Demon_Prologue_Mission3_ExchangeA_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 501:
			GameState.Get().SetBusy(busy: true);
