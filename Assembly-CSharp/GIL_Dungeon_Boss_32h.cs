...
public class GIL_Dungeon_Boss_32h : GIL_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_32h_Female_Sprite_Intro_01.prefab:7da1bbcac53b07947807b53a82b5fb0c", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_32h_Female_Sprite_EmoteResponse_01.prefab:047de08f89dd6d648ae29bd17fe4a201", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 101:
		{
			string randomLine = PopRandomLineWithChance(m_PlayerAoE);
			if (randomLine != null)
			{
		...
		case 102:
		{
			string randomLine = PopRandomLineWithChance(m_BigMinion);
			if (randomLine != null)
			{
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
			}
