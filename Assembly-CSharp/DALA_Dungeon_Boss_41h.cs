...
public class DALA_Dungeon_Boss_41h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
			case "DALA_Chu":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_41h_Male_Dwarf_IntroChu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_George":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_41h_Male_Dwarf_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_Barkeye":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_41h_Male_Dwarf_IntroMongrel_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			case "DALA_Vessina":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_41h_Male_Dwarf_IntroVessina_02, Notification.SpeechBubbleDirection.TopRight, enemyActor));
				return;
			}
			if (playerHeroCardID != "DALA_Kriziki" && playerHeroCardID != "DALA_Rakanishu" && playerHeroCardID != "DALA_Tekahn" && playerHeroCardID != "DALA_Eudora")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_PlayerBeast_01);
			break;
		case 102:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_PlayerHitsFirst_01);
			break;
		case 103:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_PlayerHunterWeapon_01);
			break;
		case 104:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_Misc_01);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "EX1_539"))
		{
			if (cardID == "EX1_538")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_BossUnleashtheHounds_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_41h_Male_Dwarf_BossKillCommand_01);
		}
	}
}
