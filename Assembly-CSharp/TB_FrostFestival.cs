...
public class TB_FrostFestival : MissionEntity
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 20:
		{
			...
			Notification popUp = NotificationManager.Get().CreatePopupText(UserAttentionBlocker.NONE, popUpPos, TutorialEntity.GetTextScale() * 2f, GameStrings.Get("POPUP_FROST_BRAWL"), convertLegacyPosition: false);
			NotificationManager.Get().DestroyNotification(popUp, 7f);
			break;
		}
		case 21:
			if (!hasPlayedArmorLine)
			{
				...
				Notification popUp = NotificationManager.Get().CreatePopupText(UserAttentionBlocker.NONE, popUpPos, TutorialEntity.GetTextScale() * 2f, GameStrings.Get("POPUP_FROST_BRAWL_ARMOR"), convertLegacyPosition: false);
				NotificationManager.Get().DestroyNotification(popUp, 7f);
