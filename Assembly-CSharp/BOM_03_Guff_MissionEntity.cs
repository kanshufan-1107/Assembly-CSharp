...
public abstract class BOM_03_Guff_MissionEntity : GenericDungeonMissionEntity
{
	...
	public bool shouldPlayBanterVO()
	{
		...
		float silenceBanterVOUntilTime = m_lastVOplayFinshtime + m_BanterVOSilenceTime;
	...
	{
		if (GetActorByCardId(minionSpeaker) == null)
		{
			yield return MissionPlayVO(brassRing, lines, InternalShouldPlayAlways, bUseBubble: false);
		}
		else
	...
	{
		Actor minionActor = GetActorByCardId(minionSpeaker);
		if (minionActor == null)
		{
			yield return MissionPlayVO(brassRing, lines, InternalShouldPlayAlways, bUseBubble: false);
		}
