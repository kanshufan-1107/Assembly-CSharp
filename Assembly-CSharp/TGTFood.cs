...
public class TGTFood : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (m_Phone && !m_Triangle.activeSelf && !(Time.timeSinceLevelLoad < m_phoneNextCheckTime))
		{
			...
			if (value && drinkEmpty && !m_isAnimating)
			{
	...
	{
		if (InputCollection.GetMouseButtonUp(0) && IsOver(m_Triangle) && !m_isAnimating)
		{
	...
	private IEnumerator RingTheBell()
	{
		...
		if (foodEmpty)
		{
			int newFoodIdx = UnityEngine.Random.Range(0, m_Food.Count);
			if (newFoodIdx == m_lastFoodIdx)
			{
				newFoodIdx = UnityEngine.Random.Range(0, m_Food.Count);
				if (newFoodIdx == m_lastFoodIdx)
				{
					newFoodIdx = m_lastFoodIdx - 1;
					if (newFoodIdx < 0)
					{
						newFoodIdx = 0;
					}
			...
			m_lastFoodIdx = newFoodIdx;
			m_CurrentFoodItem = m_Food[newFoodIdx];
			m_CurrentFoodItem.m_FSM.gameObject.SetActive(value: true);
			m_CurrentFoodItem.m_FSM.SendEvent("Birth");
		}
		if (drinkEmpty)
	...
	private void BellAnimation()
	{
		...
		{
			string soundPath = m_TriangleSoundPrefab;
			if (!string.IsNullOrEmpty(soundPath))
			{
				SoundManager.Get().LoadAndPlay(soundPath, m_Triangle);
			}
		}
	}
