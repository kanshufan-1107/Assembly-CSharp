...
public class LettuceMercenaryEquipmentDbfRecord : DbfRecord
{
	...
	public LettuceEquipmentDbfRecord LettuceEquipmentRecord => GameDbf.LettuceEquipment.GetRecord(m_lettuceEquipmentId);
	...
	{
		LettuceMercenaryEquipmentDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(LettuceMercenaryEquipmentDbfAsset)) as LettuceMercenaryEquipmentDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
			Debug.LogError($"LettuceMercenaryEquipmentDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {resourcePath}");
			return false;
		}
