...
public class LocalRenderEffect : MonoBehaviour
{
	...
	public void Render()
	{
		...
		{
			RenderTexture blurBuffer = RenderTexture.GetTemporary(m_RenderTexture.width, m_RenderTexture.height, m_RenderTexture.depth, m_RenderTexture.format);
			m_Camera.targetTexture = blurBuffer;
			m_Camera.Render();
			Sample(blurBuffer, m_RenderTexture, BlurMaterial, m_BlurAmount);
	...
	private void CreateTexture()
	{
		if (!(m_RenderTexture != null))
		{
			...
			m_RenderTexture = RenderTextureTracker.Get().CreateNewTexture((int)size.x, (int)size.y, 32, RenderTextureFormat.ARGB32);
		}
	}
	...
	private void CreateCamera()
	{
		if (!(m_Camera != null))
		{
			...
			GameObjectUtils.SetHideFlags(m_CameraGO, HideFlags.HideAndDontSave);
	...
	private Vector2 CalcTextureSize()
	{
		...
		return new Vector2(m_Resolution, (float)m_Resolution * aspect);
	}
	...
	private void CreateRenderPlane()
	{
		if (m_Width != m_PreviousWidth || m_Height != m_PreviousHeight)
		{
			...
			meshPlane.name = "TextMeshPlane";
			float halfWidth = m_Width * 0.5f;
			float halfHeight = m_Height * 0.5f;
			meshPlane.vertices = new Vector3[4]
			{
				new Vector3(0f - halfWidth, RENDER_PLANE_OFFSET, 0f - halfHeight),
				new Vector3(halfWidth, RENDER_PLANE_OFFSET, 0f - halfHeight),
				new Vector3(0f - halfWidth, RENDER_PLANE_OFFSET, halfHeight),
				new Vector3(halfWidth, RENDER_PLANE_OFFSET, halfHeight)
			};
