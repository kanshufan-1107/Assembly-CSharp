...
public abstract class TransitionPopup : MonoBehaviour
{
	...
	protected virtual void ShowPopup()
	{
		...
		args.Add("scale", new Vector3(m_endScale, m_endScale, m_endScale));
		args.Add("time", POPUP_TIME);
		args.Add("oncomplete", "PunchPopup");
		...
		args2.Add("position", base.gameObject.transform.localPosition + new Vector3(0.02f, 0.02f, 0.02f));
		args2.Add("time", 1.5f);
	...
	protected virtual void AnimateHide()
	{
		...
		Hashtable args = iTweenManager.Get().GetTweenHashTable();
		args.Add("scale", new Vector3(START_SCALE_VAL, START_SCALE_VAL, START_SCALE_VAL));
		args.Add("time", POPUP_TIME);
		if (this.OnHidden != null)
		{
			args["oncomplete"] = (Action<object>)delegate
			{
				this.OnHidden(this);
			};
		}
	...
	private void EnableFullScreenBlur()
	{
		...
		{
			m_blurEnabled = true;
			m_screenEffectsHandle.StartEffect(ScreenEffectParameters.BlurDesaturatePerspective);
		}
	}
