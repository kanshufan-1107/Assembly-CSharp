...
public class RP_Fight_02 : RP_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_TB_Hero_Illidan2_Male_NightElf_RP_Mission2_Intro02_01, Notification.SpeechBubbleDirection.BottomLeft, friendlyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_TB_Hero_Tyrande_Female_NightElf_TB_Hero_Tyrande_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 501:
			...
			yield return PlayLineAlways(enemyActor, VO_TB_Hero_Tyrande_Female_NightElf_RP_Mission2_Victory01_01);
			yield return PlayLineAlways(friendlyActor, VO_TB_Hero_Illidan2_Male_NightElf_RP_Mission2_Victory02_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 504:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(enemyActor, VO_TB_Hero_Tyrande_Female_NightElf_RP_Mission2_Loss_01);
			GameState.Get().SetBusy(busy: false);
			break;
		default:
