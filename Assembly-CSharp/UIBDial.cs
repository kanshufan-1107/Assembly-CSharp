...
public class UIBDial : PegUIElement
{
	...
	private void ShowTooltip()
	{
		if (m_EnableTooltip && m_TooltipPrefab != null && m_TooltipBone != null)
		{
			...
			GameObject tooltipObject = UnityEngine.Object.Instantiate(m_TooltipPrefab);
			LayerUtils.SetLayer(tooltipObject, GameLayer.UI);
			m_tooltip = tooltipObject.GetComponent<TooltipPanel>();
			m_tooltip.Reset();
	...
	private void LayoutDial()
	{
		...
		int maxToAdd = m_NumItemsShowingBelow + 1;
		int numAdded = 0;
		int i = m_dialItemsSelectedIndex;
		foreach (object item in m_items)
		{
			if (i >= m_DialItems.Count)
			{
				i = 0;
			}
			m_DialItems[i].SetValue(item, m_itemTextCallback(item));
			i++;
			if (++numAdded >= maxToAdd)
			{
				break;
			}
		}
	...
	private void RollUpOne()
	{
		...
		args.Add("amount", m_upRotationEuler);
		args.Add("time", m_AnimateSpinTime);
		...
		{
			object obj = m_items[m_itemsFirstVisibleIndex];
			m_DialItems[m_dialItemsFirstIndex].SetValue(obj, m_itemTextCallback(obj));
		}
		else
		{
	...
	private void RollDownOne()
	{
		...
		m_desiredOrientation = targetOrientation;
		Hashtable args = iTweenManager.Get().GetTweenHashTable();
		args.Add("amount", m_downRotationEuler);
		args.Add("time", m_AnimateSpinTime);
		args.Add("easetype", iTween.EaseType.easeOutBounce);
		args.Add("islocal", true);
		args.Add("name", "spin");
		...
		{
			object obj = m_items[m_itemsLastVisibleIndex];
			m_DialItems[m_dialItemsLastIndex].SetValue(obj, m_itemTextCallback(obj));
		}
		else
		{
			m_DialItems[m_dialItemsLastIndex].SetValue(null, string.Empty);
