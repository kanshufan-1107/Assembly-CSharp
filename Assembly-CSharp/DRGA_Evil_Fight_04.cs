...
public class DRGA_Evil_Fight_04 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_BossStartHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_EmoteResponseHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			case "DRG_026":
			case "DRG_270":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_Player_DragonAspect_01);
				break;
			case "DRG_036":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_26h_Female_Draenei_Evil_Fight_04_Player_Waxadred_01);
				break;
			}
		}
	}
