...
public class RulebookController : MonoBehaviour
{
	...
	private void ShowPopup()
	{
		...
		Vector3 centerPos = Board.Get().FindBone("FriendlyChoice").position;
		m_hoverPopupWidget.transform.localPosition = centerPos;
		Spell introSpell = m_hoverPopupWidget.GetComponentInChildren<Spell>();
		if ((bool)introSpell)
		{
			introSpell.AddStateFinishedCallback(OnIntroSpellStateFinished);
			introSpell.ActivateState(SpellStateType.BIRTH);
		}
	}
