...
public class OffScreenRenderFX : MonoBehaviour
{
	...
	private void CreateCamera()
	{
		if (offscreenFXCameraGO == null)
		{
			...
			GameObjectUtils.SetHideFlags(offscreenFXCameraGO, HideFlags.HideAndDontSave);
	...
	private void UpdateOffScreenCamera()
	{
		...
		if (ForceSize == 0f)
		{
			float renderSize = base.transform.localScale.x;
			if (UseBounds)
			{
				renderSize *= RenderBounds.size.x;
			}
			offscreenFXCamera.orthographicSize = renderSize / 2f;
		}
		else
		{
	...
	{
		Vector3 op = Vector3.up * Yoffset;
		Vector3 ox = Vector3.right * s_Xoffset;
		if (ObjectToRender != null)
		{
			ObjectToRender.transform.position += op;
		}
		ObjectToRender.transform.position += ox;
	}
}
