...
public class UNGMusic : MonoBehaviour
{
	...
	private void HandleHits()
	{
		...
		{
			if (m_MusicNotes[i] == null || !InputCollection.GetMouseButtonUp(0) || !IsOver(m_MusicNotes[i].m_CollisionObject))
			{
				continue;
			}
			...
			if (i == m_NoteSequence[m_CorrectNotesHit.Count])
			{
				...
				{
					if (fsm.FsmName == m_RewardFSMName)
					{
						fsm.SendEvent(m_RewardEvent);
					}
					else if (fsm.FsmName == m_ClickableFSMName)
					{
						fsm.SendEvent(m_ClickableEvent);
					}
				}
				m_CorrectNotesHit.Clear();
			}
