	...
	private void MonitorLoop()
	{
		while (!IsTerminated)
		{
			if (m_paused)
			{
				...
				ready.Reset();
				ready.Wait();
				ExceptionLogger.LogInfo("ANRMonitor: Unpaused");
				RefreshCalledTime();
			}
			if (!Monitor.TryEnter(m_timeLock, m_timeout))
			{
			...
			try
			{
				diff = (int)DateTime.UtcNow.Subtract(m_calledTime).TotalMilliseconds;
			}
			finally
			{
				Monitor.Exit(m_timeLock);
			}
			if (diff > m_waitLimitMilliSeconds)
			{
				if (!m_happened && ShouldReport())
				{
					ExceptionLogger.LogInfo("ANRMonitor: No update during {0} ms", diff);
			...
			{
				Thread.Sleep((diff > 0) ? (m_waitLimitMilliSeconds - diff) : m_maxSleepTimeMilliSeconds);
			}
		}
	}
