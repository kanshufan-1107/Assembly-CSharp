...
public class MoveMinionHoverTarget : MonoBehaviour
{
	...
	public void DropCardOnHoverTarget(Card heldCard)
	{
		...
		if (SetSelectedOption(heldEntity, out var mainOptionPlayErrorType, out var mainOptionPlayErrorParam, out var targetPlayErrorType, out var targetPlayErrorParam))
		{
			state.SetSelectedOptionTarget(heldEntity.GetEntityId());
			state.SendOption();
			return;
		}
