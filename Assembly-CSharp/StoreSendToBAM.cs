...
public class StoreSendToBAM : UIBPopup
{
	...
	public void Show(MoneyOrGTAPPTransaction moneyOrGTAPPTransaction, BAMReason reason, string errorCode, bool fromPreviousPurchase)
	{
		...
		{
			ProductId productId = ProductId.CreateFrom(m_moneyOrGTAPPTransaction.PMTProductID.GetValueOrDefault());
			m_sendToBAMRoot.transform.position = m_sendToBAMRootWithSummaryBone.position;
			m_miniSummary.SetDetails(productId, 1);
	...
	private void UpdateText()
	{
		...
		string messageText = sendToBAMText.GetDetails();
		if (!string.IsNullOrEmpty(m_errorCode))
		{
			messageText = messageText + " " + GameStrings.Format("GLUE_STORE_FAIL_DETAILS_ERROR_CODE", m_errorCode);
		}
		messageText += "\n\n";
		messageText += sendToBAMText.GetGoToURLDetails(m_okayButton.m_ButtonText.Text);
	...
	private void LayoutMessageText()
	{
		...
		float midSectionHeight = TransformUtil.ComputeOrientedWorldBounds(m_midSection).Extents[2].magnitude * 2f;
		Bounds msgTextBounds = m_messageText.GetTextWorldSpaceBounds();
		TransformUtil.SetLocalScaleZ(m_midSection, msgTextBounds.size.z / midSectionHeight);
