...
public class ULDA_Dungeon_Boss_19h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_19h_Male_TitanWatcher_IntroBrannResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (missionEvent == 101)
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_19h_Male_TitanWatcher_PlayerTrigger_5AtkorGreaterWeapon_01);
		}
		else
		{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_721"))
		{
			if (cardID == "ULDA_208")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_19h_Male_TitanWatcher_PlayerTriggerTitanRingTreasure_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_19h_Male_TitanWatcher_PlayerTrigger_Colossus_of_the_Moon_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "CS2_108"))
		{
			if (cardID == "ULD_256")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_19h_Male_TitanWatcher_BossTriggerReinforcements_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_19h_Male_TitanWatcher_BossTriggerExecute_01);
		}
	}
}
