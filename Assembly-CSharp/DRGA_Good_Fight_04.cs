...
public class DRGA_Good_Fight_04 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			if (playerHeroCardID == "HERO_09b")
			{
				Gameplay.Get().StartCoroutine(PlayMultipleVOLinesForEmotes(emoteType, emoteSpell));
			}
			else if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_BossStart_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_BossStartHeroic_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_21h_EmoteResponse, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected IEnumerator PlayMultipleVOLinesForEmotes(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		Actor friendlyActor = GameState.Get().GetFriendlySidePlayer().GetHeroCard()
			.GetActor();
		yield return PlayLineAlways(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_BossStartLazul_01);
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 100:
			...
			break;
		case 104:
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_BossAttack_01);
			break;
		case 105:
			yield return PlayLineAlways(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_EVENT_Summoning01_01);
			break;
		case 106:
			yield return PlayLineAlways(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_EVENT_Summoning02_01);
			if (!m_Heroic)
			{
		...
		case 108:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_EVENT_Summoning04_01);
			if (!m_Heroic)
			{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		case "DRGA_BOSS_03t2":
			if (!m_Heroic)
			{
				yield return PlayAndRemoveRandomLineOnlyOnce(friendlyActor, VO_DRGA_BOSS_03h_Male_Murloc_Good_Fight_04_EVENT_FromTheDepths);
			}
			break;
		case "OG_133":
		case "OG_280":
		case "OG_042":
		case "OG_134":
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_Player_OldGod_01);
			break;
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "DRGA_BOSS_09t":
				yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_Boss_PuppeteerLines);
				break;
			case "DRGA_BOSS_09t2":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_EVENT_OldGodExperiments_01);
				break;
			case "DAL_011":
				yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_09h_Female_Troll_Good_Fight_04_Player_LazulScheme_01);
				break;
			}
		}
	}
	...
	public override void OnPlayThinkEmote()
	{
		...
		string enemyHeroCardID = GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetCardId();
		if (thinkEmoteBossThinkChancePercentage > randomThink && m_BossIdleLines != null && m_BossIdleLines.Count != 0 && enemyHeroCardID == "DRGA_BOSS_09h")
		{
