...
public class DisenchantBar
{
	...
	public void UpdateVisuals(int totalNumCards)
	{
		...
		{
			MeshFilter meshFilter = m_barFrameMesh.GetComponent<MeshFilter>();
			if (m_numGoldCards > 0)
			{
				...
				meshFilter.mesh = MassDisenchant.Get().m_rarityBarGoldMesh;
			...
			else
			{
				...
				TransformUtil.SetLocalPosX(m_numCardsText, 2.902672f);
				meshFilter.mesh = MassDisenchant.Get().m_rarityBarNormalMesh;
				m_barFrameMesh.SetMaterial(MassDisenchant.Get().m_rarityBarNormalMaterial);
			}
		}
		float percentFill = (((float)totalNumCards > 0f) ? ((float)m_numCards / (float)totalNumCards) : 0f);
		SetPercent(percentFill);
	}
