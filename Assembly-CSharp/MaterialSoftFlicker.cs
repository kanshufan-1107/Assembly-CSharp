...
public class MaterialSoftFlicker : MonoBehaviour
{
	...
	private void Update()
	{
		float noise = Mathf.PerlinNoise(random, Time.time * m_timeScale);
		m_renderer.GetMaterial().SetColor("_TintColor", new Color(m_color.r, m_color.g, m_color.b, Mathf.Lerp(minIntensity, maxIntensity, noise)));
	}
}
