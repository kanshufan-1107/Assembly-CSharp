...
public class CardBackHelperBubbleLevel : MonoBehaviour
{
	...
	private void Update()
	{
		float startTiltX = NormalizeRotation(base.gameObject.transform.eulerAngles.x);
		float startTiltY = NormalizeRotation(base.gameObject.transform.eulerAngles.y - 180f);
		float startTiltZ = NormalizeRotation(base.gameObject.transform.eulerAngles.z);
		m_objectTilt = startTiltX + startTiltY + startTiltZ;
