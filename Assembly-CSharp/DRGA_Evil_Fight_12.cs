...
public class DRGA_Evil_Fight_12 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType != EmoteType.START && MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_24h_Male_Dragon_Evil_Fight_12_EmoteResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRG_610_Male_Dragon_Threaten_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 104:
			...
			break;
		case 107:
			yield return PlayLineOnlyOnce(enemyActor, VO_DRGA_BOSS_24h_Male_Dragon_Evil_Fight_12_BossAttack_01);
			break;
		case 108:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_VO_DRGA_BOSS_07h_Male_Ethereal_Evil_Fight_12_HeroPowerLines);
			break;
		case 109:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_VO_DRGA_BOSS_24h_Male_Dragon_Evil_Fight_12_Boss_HeroPowerLines);
			break;
		case 110:
			if (!m_Heroic)
			{
		...
		case 113:
			...
			yield return PlayLineAlways(enemyActor, VO_DRG_650_Male_Dragon_Start_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 114:
			yield return PlayLineAlways(enemyActor, VO_DRGA_BOSS_24h_Male_Dragon_Evil_Fight_12_BossStart_01);
			if (!m_Heroic)
			{
