...
public class GoldReward : Reward
{
	...
	protected override void OnDataSet(bool updateVisuals)
	{
		...
		if (!(base.Data is GoldRewardData goldRewardData))
		{
			Debug.LogWarning($"goldRewardData.SetData() - data {base.Data} is not GoldRewardData");
			return;
		}
		string rewardHeadline = GameStrings.Get("GLOBAL_REWARD_GOLD_HEADLINE");
		string rewardDetails = goldRewardData.Amount.ToString();
		...
		case NetCache.ProfileNotice.NoticeOrigin.IGR:
			...
			{
				string date = GameStrings.Format("GLOBAL_CURRENT_DATE", goldRewardData.Date);
				rewardSource = GameStrings.Format("GLOBAL_REWARD_GOLD_SOURCE_IGR_DATED", date);
			}
			else
			{
				rewardSource = GameStrings.Get("GLOBAL_REWARD_GOLD_SOURCE_IGR");
			}
			break;
		}
