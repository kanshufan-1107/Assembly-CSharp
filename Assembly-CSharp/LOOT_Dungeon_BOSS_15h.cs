...
public class LOOT_Dungeon_BOSS_15h : LOOT_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_15h_Male_Human_Intro_01.prefab:eac2e5310989138459bcaccb9cf1e262", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			if (enemyHeroCardID == "LOOTA_BOSS_15h")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_15h_Male_Human_EmoteResponse_01.prefab:418380d3659570c4c97e6c1c118a9f6f", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			if (enemyHeroCardID == "LOOTA_BOSS_32h")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_32h_Male_Human_EmoteResponse_01.prefab:8ef89068a08ab764d9edd2ffd51fecca", Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 101:
		{
			...
			if (m_GeorgeTaggedInLines.Count != 0 && chanceVO >= randomNum)
			{
				string randomLine = m_GeorgeTaggedInLines[Random.Range(0, m_GeorgeTaggedInLines.Count)];
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
				m_GeorgeTaggedInLines.Remove(randomLine);
				yield return null;
			}
			break;
		}
		case 102:
		{
			int chanceVO = 100;
			int randomNum = Random.Range(0, 100);
			if (m_KarlTaggedInLines.Count != 0 && chanceVO >= randomNum)
			{
				string randomLine = m_KarlTaggedInLines[Random.Range(0, m_KarlTaggedInLines.Count)];
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
				m_KarlTaggedInLines.Remove(randomLine);
				yield return null;
			}
			break;
		}
