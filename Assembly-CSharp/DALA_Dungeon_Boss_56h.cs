...
public class DALA_Dungeon_Boss_56h : DALA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_56h_Female_Kobold_IntroGeorge_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "DALA_Rakanishu")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DALA_BOSS_56h_Female_Kobold_IntroRakanishu_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID != "DALA_Squeamlish" && playerHeroCardID != "DALA_Vessina")
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 101:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_BossTreasure_01);
			break;
		case 102:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerWeak);
			break;
		case 103:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPower);
			break;
		case 104:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_HeroPowerStrong);
			break;
		case 105:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_HeroPowerDoomsayer_01);
			break;
		case 106:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_HeroPowerTreasure_01);
			break;
		case 107:
			yield return PlayAndRemoveRandomLineOnlyOnce(enemyActor, m_ReturnMinion);
			break;
		case 108:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_Misc_01);
			break;
		case 109:
			yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_Misc_02);
			break;
		default:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "EX1_126":
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_BossBetrayal_01);
				break;
			case "AT_033":
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_BossBurgle_01);
				break;
			case "NEW1_004":
				yield return PlayLineOnlyOnce(enemyActor, VO_DALA_BOSS_56h_Female_Kobold_BossVanish_01);
				break;
			}
		}
	}
