...
public class InnKeepersSpecialAd
{
	...
	{
		string dataToHash = GameAction + Contents + Title + SubTitle + ButtonText + Link;
		using SHA256 hasher = SHA256.Create();
		StringBuilder stringBuilder = new StringBuilder();
		byte[] array = hasher.ComputeHash(Encoding.UTF8.GetBytes(dataToHash));
		foreach (byte hashByte in array)
		{
