...
public class BOM_03_Guff_Fight_05 : BOM_03_Guff_Dungeon
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		{
		case 514:
			yield return MissionPlayVO(friendlyActor, VO_Story_Hero_Guff_Male_Tauren_Story_Guff_Mission5Intro_01);
		...
		case 505:
			GameState.Get().SetBusy(busy: true);
			yield return MissionPlayVO(friendlyActor, VO_Story_Hero_Guff_Male_Tauren_Story_Guff_Mission5Victory_01);
			GameState.Get().SetBusy(busy: false);
			break;
		case 517:
		{
			float m_randomIdleSpeaker = Random.Range(0f, 3f);
			if (m_randomIdleSpeaker < 1f && (bool)GetActorByCardId("WC_034t6"))
			{
				yield return MissionPlayVO("WC_034t6", VO_Story_Hero_SneakyAdventurer_Female_Forsaken_Story_Guff_Mission5Idle_02);
			}
			else if (m_randomIdleSpeaker < 2f && (bool)GetActorByCardId("WC_034t8"))
			{
