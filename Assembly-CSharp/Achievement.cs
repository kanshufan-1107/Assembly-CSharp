...
public class Achievement
{
	...
	private EventTimingType m_eventTrigger;
	...
	public EventTimingType EventTrigger => m_eventTrigger;
	...
	public bool IsFriendlyChallengeQuest
	{
		get
		{
			if (m_gameMode == Assets.Achieve.GameMode.FRIENDLY)
			{
				return true;
			}
			return false;
	...
	public void SetEventTrigger(EventTimingType eventType)
	{
	...
	public void OnAchieveNotification(AchievementNotification notification)
	{
		...
		achieveData.CompletionCount = CompletionCount;
		achieveData.Progress = Progress;
		achieveData.Active = Active;
		achieveData.DoNotAck = !CanBeAcknowledged;
		achieveData.DateCompleted = TimeUtils.FileTimeUtcToPegDate(DateCompleted);
		achieveData.DateGiven = TimeUtils.FileTimeUtcToPegDate(DateGiven);
		achieveData.AckProgress = AcknowledgedProgress;
		...
		if (notification.NewAchievement)
		{
			...
			achieveData.AckProgress = NEW_ACHIEVE_ACK_PROGRESS;
			achieveData.Progress = 0;
		}
		achieveData.Progress += notification.Amount;
		if (notification.Complete)
		{
			achieveData.Progress = MaxProgress;
		...
		{
			OnAchieveData(achieveData);
		}
		else
		{
			UpdateActiveAchieve(achieveData);
		}
	}
	...
	public AchieveRegionDataDbfRecord GetCurrentRegionData()
	{
		...
		AchieveRegionDataDbfRecord data = GameDbf.AchieveRegionData.GetRecord((AchieveRegionDataDbfRecord dbf) => dbf.AchieveId == ID && dbf.Region == (int)currentRegion);
		if (data == null)
		{
			data = GameDbf.AchieveRegionData.GetRecord((AchieveRegionDataDbfRecord dbf) => dbf.AchieveId == ID && dbf.Region == 0);
		}
		return data;
	}
