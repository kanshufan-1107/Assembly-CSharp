...
public class TurnStartManager : MonoBehaviour
{
	...
	public void NotifyOfExhaustedChange(Card card, TagDelta tagChange)
	{
		...
		m_exhaustedChangesToHandle.Add(change);
	}
	...
	private int GetCurrentAlternativeAppearanceIndex()
	{
		...
		return state.GetGameEntity()?.GetTag(GAME_TAG.TURN_INDICATOR_ALTERNATIVE_APPEARANCE) ?? 0;
	}
	...
	private bool IsTurnStartIndicatorDisabled()
	{
		...
		GameEntity game = state.GetGameEntity();
		if (game == null)
		{
			return false;
		}
		return game.GetTag(GAME_TAG.DISABLE_TURN_INDICATORS) > 0;
	}
	...
	private IEnumerator InstantiateTurnStartIndicator()
	{
		...
		{
			int alternativeAppearanceIndex = GetCurrentAlternativeAppearanceIndex();
			if (alternativeAppearanceIndex == 0)
			{
				m_turnStartInstance = UnityEngine.Object.Instantiate(m_turnStartPrefab);
			}
			else if (m_alternativeTurnStartPrefab.Count >= alternativeAppearanceIndex)
			{
				m_turnStartInstance = UnityEngine.Object.Instantiate(m_alternativeTurnStartPrefab[alternativeAppearanceIndex - 1]);
			}
			if (m_turnStartInstance != null)
			{
			...
			{
				Debug.LogError($"TurnStartManager.InstantiateTurnStartIndicator() - FAILED to instantiate turn start prefab for appearance {alternativeAppearanceIndex}");
			}
		}
	}
	...
	{
		Spell extraTurnSpell = m_friendlyExtraTurnSpellInstance;
		if (!isFriendly)
		{
			extraTurnSpell = m_opponentExtraTurnSpellInstance;
		}
	...
	{
		int index = m_cardsToDraw.IndexOf(card);
		if (index >= 0)
		{
			Card[] cardsToDraw = m_cardsToDraw.GetRange(0, index + 1).ToArray();
			m_cardsToDraw.RemoveRange(0, index + 1);
			StartCoroutine(DrawCardsImmediatelyWithTiming(cardsToDraw));
		}
	}
	...
	private bool HasActionsAfterCardDraw()
	{
		...
		Network.EntityChoices friendlyChoices = GameState.Get().GetFriendlyEntityChoices();
		if (friendlyChoices != null && friendlyChoices.ChoiceType == CHOICE_TYPE.GENERAL)
		{
			return true;
		}
		return false;
	...
	{
		foreach (CardChange change in m_exhaustedChangesToHandle)
		{
