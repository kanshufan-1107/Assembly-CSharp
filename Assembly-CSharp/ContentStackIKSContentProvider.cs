...
public class ContentStackIKSContentProvider : BaseIKSContentProvider
{
	...
	{
		m_connect.InitializeURL("innkeeper_special", Vars.Key("ContentStack.Env").GetStr("production"), Localization.GetBnetLocaleName(), BattleNet.GetCurrentRegion() == BnetRegion.REGION_CN, "IKS_LAST_STORED_RESPONSE", Vars.Key("ContentStack.CacheAge").GetInt(0));
	...
	private T GetAttribute<T>(JsonNode adNode, AdAttributes attr)
	{
		...
		{
			JsonNode media = adNode["in_game_ad"] as JsonNode;
			return BaseIKSContentProvider.GetStringFromNode<T>("url", media);
		}
