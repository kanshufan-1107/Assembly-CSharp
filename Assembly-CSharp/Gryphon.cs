...
public class Gryphon : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (m_UniversalInputManager != null)
		{
			...
			{
				poke = InputCollection.GetMouseButtonDown(0);
			}
			if (poke)
			{
	...
	{
		List<Vector3> lookAtPostions = new List<Vector3>();
		...
		foreach (ZonePlay item in zoneManager.FindZonesOfType<ZonePlay>())
		{
			foreach (Card card in item.GetCards())
			{
				...
				lookAtPostions.Add(card.transform.position);
			}
		}
		if (Random.Range(0, 100) < m_LookAtHeroesPercent)
		{
			foreach (ZoneHero item2 in ZoneMgr.Get().FindZonesOfType<ZoneHero>())
			{
				foreach (Card card2 in item2.GetCards())
				{
					...
					lookAtPostions.Add(card2.transform.position);
				}
			}
		}
		if (lookAtPostions.Count > 0)
		{
			int randomIdx = Random.Range(0, lookAtPostions.Count);
			m_LookAtPosition = lookAtPostions[randomIdx];
		}
		else
		{
			PlayAniamtion();
	...
	private void AniamteHead()
	{
		if (m_CurrentBaseLayerState.fullPathHash != lookState && m_CurrentBaseLayerState.fullPathHash != cleanState && m_CurrentBaseLayerState.fullPathHash != screechState)
		{
			Quaternion lookRotation = Quaternion.LookRotation(m_LookAtPosition - m_HeadBone.position);
			m_HeadBone.rotation = Quaternion.Slerp(m_HeadBone.rotation, lookRotation, Time.deltaTime * m_HeadRotationSpeed);
		}
	}
}
