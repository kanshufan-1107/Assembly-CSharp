...
public class SpriteSheet : MonoBehaviour
{
	...
	private void Update()
	{
		if (m_Old_Mode)
		{
			int idx = (int)(Time.time * m_fps % (float)(_uvTieX * _uvTieY));
			if (idx != m_LastIdx)
			{
				Material material = m_renderer.GetMaterial();
				material.mainTextureOffset = new Vector2((float)(idx % _uvTieX) * m_Size.x, 1f - m_Size.y - (float)(idx / _uvTieY) * m_Size.y);
