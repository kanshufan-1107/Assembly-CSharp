...
public abstract class DeckRule
{
	public enum RuleType
	{
		...
		SIDEBOARD_CARD_COUNT_LIMIT,
		SIDEBOARD_HAS_TAG_VALUE,
		PLAYER_OWNS_DECK_TEMPLATE,
		TOURIST_LIMIT,
		UNKNOWN
	}
	...
	public List<HashSet<string>> GetSubsets()
	{
		return m_subsets;
	}
	...
	public static DeckRule GetRule(DeckRulesetRuleDbfRecord record)
	{
		return record.RuleType switch
		{
			...
			DeckRulesetRule.RuleType.SIDEBOARD_HAS_TAG_VALUE => new DeckRule_SideboardHasTagValue(record), 
			DeckRulesetRule.RuleType.EDITING_DECK_EXTRA_CARD_COUNT => new DeckRule_EditingDeckExtraCardCount(record), 
			DeckRulesetRule.RuleType.DEATHKNIGHT_RUNE_LIMIT => new DeckRule_DeathKnightRuneLimit(record), 
			DeckRulesetRule.RuleType.SIDEBOARD_CARD_COUNT_LIMIT => new DeckRule_SideboardCardCount(record), 
			DeckRulesetRule.RuleType.PLAYER_OWNS_DECK_TEMPLATE => new DeckRule_PlayerOwnsDeckTemplate(record), 
			DeckRulesetRule.RuleType.TOURIST_LIMIT => new DeckRule_TouristLimit(record), 
