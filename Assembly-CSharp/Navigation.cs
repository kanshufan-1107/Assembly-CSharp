...
public static class Navigation
{
	...
	public static bool GoBack()
	{
		...
		NavigateBackHandler handler = m_history.Peek();
		if (handler())
		{
			if (m_history.Count > 0 && handler == m_history.Peek())
			{
				m_history.Pop();
			}
			else if (m_history.Contains(handler))
			{
				Log.All.PrintWarning("Navigation tried to remove handler and failed, but the handler exists further down the stack! Perhaps something went wrong, like a new scene added itself to the top of the stack in its Awake? Handler to remove: {0}", StackEntryToString(handler));
			}
			if (NAVIGATION_DEBUG)
			{
				DumpStack();
	...
	public static void DumpStack()
	{
		...
		int count = 0;
		foreach (NavigateBackHandler entry in m_history)
		{
			Debug.Log($"{count}: {StackEntryToString(entry)}\n");
			count++;
		}
	}
