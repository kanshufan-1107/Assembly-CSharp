...
public abstract class TB_MagicalGuardians_MissionEntity : GenericDungeonMissionEntity
{
	...
	public TB_MagicalGuardians_MissionEntity()
	{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlayCustomEmoteResponse(emoteType, emoteSpell));
		}
	}

	public virtual IEnumerator PlayCustomEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		yield return null;
	}
	...
	{
		float HeroExplosionAnimationTime = 3f;
		yield return new WaitForSeconds(HeroExplosionAnimationTime);
		yield return HandleMissionEventWithTiming(VOTriggerID);
	}
	...
	public bool shouldPlayBanterVO()
	{
		...
		float silenceBanterVOUntilTime = m_lastVOplayFinishtime + m_BanterVOSilenceTime;
		if (time > silenceBanterVOUntilTime)
		{
			return true;
		}
		return false;
	...
	protected IEnumerator MissionPlayVO(AssetReference brassRing, List<string> lines, ShouldPlay shouldPlay, bool bUseBubble = true, bool bPlayOrder = true)
	{
		bool removePickedLine = false;
		if (shouldPlay() == ShouldPlayValue.Once && !m_forceAlwaysPlayLine)
		{
		...
		yield return MissionPlayVO(line: (!bPlayOrder) ? PopRandomLine(lines, removePickedLine) : PopNextLine(lines, removePickedLine), brassRing: brassRing, bUseBubble: bUseBubble, shouldPlay: shouldPlay);
	}

	protected IEnumerator MissionPlayVO(AssetReference brassRing, List<string> lines, Actor minionOverride, ShouldPlay shouldPlay, bool bUseBubble = true, bool bPlayOrder = true)
	{
	...
	{
		yield return MissionPlayVO(FindActorInPlayByDesignCode(minionSpeaker), lines, InternalShouldPlayAlways, bUseBubble: false);
	}

	public IEnumerator MissionPlayVOOnce(string minionSpeaker, string line)
	{
		yield return MissionPlayVO(FindActorInPlayByDesignCode(minionSpeaker), line, bUseBubble: true, InternalShouldPlayOnce);
	}

	public IEnumerator MissionPlayVO(string minionSpeaker, List<string> lines)
	{
		yield return MissionPlayVO(FindActorInPlayByDesignCode(minionSpeaker), lines, InternalShouldPlayAlways);
	}

	public IEnumerator MissionPlayVOOnce(string minionSpeaker, List<string> lines)
	{
		yield return MissionPlayVO(FindActorInPlayByDesignCode(minionSpeaker), lines, base.InternalShouldPlayOnlyOnce);
	}
	...
	{
		yield return MissionPlayVO(FindActorInPlayByDesignCode(minionSpeaker), lines, base.InternalShouldPlayOnlyOnce, bUseBubble: false);
	}
	...
	{
		Actor minionActor = FindActorInPlayByDesignCode(minionSpeaker);
		if (minionActor == null)
		{
			yield return MissionPlayVO(brassRing, lines, InternalShouldPlayAlways, bUseBubble: false);
		}
		...
		{
			yield return MissionPlayVO(minionActor, lines, base.InternalShouldPlayOnlyOnce, bUseBubble: false);
		}
	}
