...
public class BoH_Thrall_01 : BoH_Thrall_Dungeon
{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 328:
		{
			...
			NotificationManager.Get().DestroyNotification(popup2, 3.5f);
			break;
		}
		case 428:
		{
			...
			NotificationManager.Get().DestroyNotification(popup, 3.5f);
			break;
		}
		default:
	...
	protected override IEnumerator HandleStartOfTurnWithTiming(int turn)
	{
		...
		{
			yield return MissionPlaySound(enemyActor, Low_Drumroll);
			Notification popup = NotificationManager.Get().CreatePopupText(UserAttentionBlocker.NONE, popUpPos, TutorialEntity.GetTextScale() * popUpScale, GameStrings.Get(m_popUpInfo[m_missionEventBannerID][0]), convertLegacyPosition: false, NotificationManager.PopupTextType.FANCY);
			NotificationManager.Get().DestroyNotification(popup, 6.5f);
