...
public class AdventureGenericButton : PegUIElement
{
	...
	public void SetPortraitTexture(string textureAssetPath, int index, string mattexprop)
	{
		...
		if (CheckValidMaterialProperties(matprop))
		{
			m_PortraitLoaded = false;
			if (!AssetLoader.Get().LoadAsset<Texture>(textureAssetPath, OnLoadPortraitTextureAttempted, matprop))
			{
				OnLoadPortraitTextureAttempted(textureAssetPath, null, matprop);
			}
		}
	}
	...
	public void SetPortraitTiling(Vector2 tiling, int index, string mattexprop)
	{
		...
		if (CheckValidMaterialProperties(matprop))
		{
			m_PortraitRenderer.GetMaterial(matprop.m_MaterialIndex).SetTextureScale(matprop.m_MaterialPropertyName, tiling);
		}
	}
	...
	public void SetPortraitOffset(Vector2 offset, int index, string mattexprop)
	{
		...
		if (CheckValidMaterialProperties(matprop))
		{
			m_PortraitRenderer.GetMaterial(matprop.m_MaterialIndex).SetTextureOffset(matprop.m_MaterialPropertyName, offset);
		}
	}

	private void OnLoadPortraitTextureAttempted(AssetReference assetRef, AssetHandle<Texture> loadedTexture, object callbackData)
	{
		using (loadedTexture)
		{
			m_PortraitLoaded = true;
			MaterialProperties matprop = callbackData as MaterialProperties;
			if (!loadedTexture)
			{
			...
			m_PortraitRenderer.GetMaterial(matprop.m_MaterialIndex).SetTexture(matprop.m_MaterialPropertyName, m_portraitTexture);
		}
	}
