...
public class QuestDialogOnReceivedDbfRecord : DbfRecord
{
	...
	{
		QuestDialogOnReceivedDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(QuestDialogOnReceivedDbfAsset)) as QuestDialogOnReceivedDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
			Debug.LogError($"QuestDialogOnReceivedDbfAsset.LoadRecordsFromAsset() - failed to load records from assetbundle: {resourcePath}");
			return false;
		}
