...
public class KAR12_Portals : KAR_MissionEntity
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			...
			case "KARA_13_01":
			case "KARA_13_01H":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_Nazra_Female_Orc_FinalNazraEmoteResponse_01.prefab:6213bb0b535356b40b360716d732ed83", Notification.SpeechBubbleDirection.TopRight, enemyActor));
				break;
			case "KARA_13_06":
			case "KARA_13_06H":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_Malchezaar_Male_Demon_FinalAltOpening_01.prefab:9902000abbbc66348b13baaefad5a6ef", Notification.SpeechBubbleDirection.TopRight, enemyActor));
				break;
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 14:
		{
			...
			GameState.Get().SetBusy(busy: true);
			GameObject medivhObj = GameObject.Find("Medivh_Hero");
			if (medivhObj == null)
			{
			...
			Actor medivhActor = medivhObj.GetComponent<Actor>();
			if (medivhActor == null)
			{
			...
			else
			{
				medivhActor.SetEntity(GameState.Get().GetFriendlySidePlayer().GetHeroCard()
					.GetEntity());
				yield return PlayCriticalLine(medivhActor, "VO_Medivh_Male_Human_FinalThirdSequence_03.prefab:9a08107f3d877f44bb4ebef53db3c708");
				GameState.Get().SetBusy(busy: false);
			}
			break;
		}
		case 16:
		{
			...
			GameState.Get().SetBusy(busy: true);
			GameObject medivhObj2 = GameObject.Find("Medivh_Hero");
			if (medivhObj2 == null)
			{
				Log.All.PrintError("Could not find Medivh_Hero gameObject");
				GameState.Get().SetBusy(busy: false);
			...
			Actor medivhActor2 = medivhObj2.GetComponent<Actor>();
			if (medivhActor2 == null)
			{
				Log.All.PrintError("Could not find actor component for Medivh_Hero gameObject");
				GameState.Get().SetBusy(busy: false);
				break;
			}
			medivhActor2.SetEntity(GameState.Get().GetFriendlySidePlayer().GetHeroCard()
				.GetEntity());
			yield return new WaitForSeconds(1f);
			yield return PlayCriticalLine(medivhActor2, "VO_Medivh_Male_Human_FinalThirdSequence_01.prefab:f7616460d19fffe4682697c0dd03d2b6");
			GameState.Get().SetBusy(busy: false);
			break;
		}
		case 17:
	...
	protected override IEnumerator HandleStartOfTurnWithTiming(int turn)
	{
		...
		string enemyHeroCardID = GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetCardId();
		if (enemyHeroCardID == "KARA_13_01" || enemyHeroCardID == "KARA_13_01H")
		{
			switch (turn)
			{
			case 1:
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		case "KARA_13_11":
			...
			yield return PlayBossLine(enemyActor, "VO_Malchezaar_Male_Demon_FinalMalchezaarEmoteResponse_01.prefab:80deff75e71de3941ac8b8d4167fb814");
			break;
		case "KARA_13_12":
			m_playedLines.Add(cardID);
			yield return PlayBossLine(enemyActor, "VO_Malchezaar_Male_Demon_Brawl_06.prefab:7ebc074519c4bee4b9ee2dc3a022cb8a");
			break;
		case "KARA_00_02":
		case "KARA_13_13H":
			if (hero.GetTag(GAME_TAG.TAG_SCRIPT_DATA_NUM_1) > 2)
			{
				m_playedLines.Add(cardID);
				GameState.Get().SetBusy(busy: true);
				yield return PlayBossLine(enemyActor, "VO_Malchezaar_Male_Demon_FinalMalchezaarHeroPower_01.prefab:35d5e0210c690e244830efff7b029b56");
				GameState.Get().SetBusy(busy: false);
			}
			break;
		case "EX1_312":
			m_playedLines.Add(cardID);
			yield return PlayBossLine(enemyActor, "VO_Malchezaar_Male_Demon_FinalThirdSequence_04.prefab:2fd08853587f35e47898939b971b282c");
			break;
		}
	}
	...
	public override IEnumerator DoCustomIntro(Card friendlyHero, Card enemyHero, HeroLabel friendlyHeroLabel, HeroLabel enemyHeroLabel, GameStartVsLetters versusText)
	{
		...
		{
			m_introSpellInstance = SpellManager.Get().GetSpell("Nazra_PreMissionSummon.prefab:22f4f2bf8acd31541b4ce82bab9a1907");
			...
			Notification.SpeechBubbleDirection speechBubDir = Notification.SpeechBubbleDirection.TopRight;
			if ((bool)UniversalInputManager.UsePhoneUI)
			{
				speechBubDir = Notification.SpeechBubbleDirection.TopLeft;
			}
			yield return Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_Nazra_Female_Orc_FinalNazraHeroPower_01.prefab:2e9958d6650ca08469a566b41f9a7df0", speechBubDir, enemyActor, 3f, 1f, parentBubbleToActor: false, delayCardSoundSpells: false, 0.6f));
			if ((bool)UniversalInputManager.UsePhoneUI)
			{
