...
public class IdleSuperSpell : SuperSpell
{
	...
	{
		Actor sourceActor = GetSourceCard().GetActor();
		m_idleSpellInstance = SpellManager.Get().GetSpell(m_idleSpell);
		SpellUtils.SetCustomSpellParent(m_idleSpellInstance, this);
		if (sourceActor != null)
		{
			TransformUtil.AttachAndPreserveLocalTransform(m_idleSpellInstance.transform, sourceActor.transform);
		}
	...
	private bool TryIdleFinish()
	{
		if (m_taskList != null && !m_taskList.IsEndOfBlock())
		{
			...
			{
				if (taskList.HasTasks() && !taskList.AreTasksComplete())
				{
					return false;
				}
			}
