	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 1:
			if (once && m_LichKingCard != null)
			{
				...
				Notification.SpeechBubbleDirection correctOrientation = (m_LichKingActor.GetEntity().IsControlledByFriendlySidePlayer() ? Notification.SpeechBubbleDirection.BottomLeft : Notification.SpeechBubbleDirection.TopRight);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LichKing_Male_Human_Brawl_01.prefab:df6d7692c0d3d8c4aab91a2eec0a3d9f", correctOrientation, m_LichKingActor));
		...
		case 3:
			...
			{
				GameState.Get().SetBusy(busy: true);
				Notification.SpeechBubbleDirection correctOrientation3 = (m_LichKingActor.GetEntity().IsControlledByFriendlySidePlayer() ? Notification.SpeechBubbleDirection.BottomLeft : Notification.SpeechBubbleDirection.TopRight);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LichKing_Male_Human_Brawl_03.prefab:12fd2a3bd4b0945448667db58a95f32b", correctOrientation3, m_LichKingActor));
				GameState.Get().SetBusy(busy: false);
			}
			break;
		case 5:
			if (m_LichKingCard != null)
			{
				once = false;
				GameState.Get().SetBusy(busy: true);
				Notification.SpeechBubbleDirection correctOrientation2 = (m_LichKingActor.GetEntity().IsControlledByFriendlySidePlayer() ? Notification.SpeechBubbleDirection.BottomLeft : Notification.SpeechBubbleDirection.TopRight);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LichKing_Male_Human_Brawl_05.prefab:96e0b55b99289824ebbae0d6201e936c", correctOrientation2, m_LichKingActor));
		...
		case 20:
			if (banner != null)
			{
				banner.SetName(GameStrings.Get("FB_LK_HERO_02"));
			}
			break;
		case 21:
			if (banner != null)
			{
				banner.SetName(GameStrings.Get("FB_LK_HERO_03"));
			}
			break;
		case 22:
			if (banner != null)
			{
				banner.SetName(GameStrings.Get("FB_LK_HERO_04"));
			}
			break;
		case 23:
			if (banner != null)
			{
				banner.SetName(GameStrings.Get("FB_LK_HERO_05"));
			}
			break;
		case 24:
			if (banner != null)
			{
				banner.SetName(GameStrings.Get("FB_LK_HERO_01"));
			}
			break;
		}
	}
