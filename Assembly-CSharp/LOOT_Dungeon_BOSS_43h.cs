...
public class LOOT_Dungeon_BOSS_43h : LOOT_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_43h_Female_Djinn_Intro_01.prefab:d807d173b15d7a349ac7633f4d6d49cb", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_LOOTA_BOSS_43h_Female_Djinn_EmoteResponse_01.prefab:b0b146264452748448f38ba8653c4152", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 101:
		{
			...
			if (m_NormalHeroLines.Count != 0 && chanceVO >= randomNum)
			{
				string randomLine = m_NormalHeroLines[Random.Range(0, m_NormalHeroLines.Count)];
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
				m_NormalHeroLines.Remove(randomLine);
				yield return null;
			}
			break;
		}
		case 102:
		{
			int chanceVO = 20;
			int randomNum = Random.Range(0, 100);
			if (m_BurnedHeroLines.Count != 0 && chanceVO >= randomNum)
			{
				string randomLine = m_BurnedHeroLines[Random.Range(0, m_BurnedHeroLines.Count)];
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
				m_BurnedHeroLines.Remove(randomLine);
				yield return null;
			}
			break;
		}
