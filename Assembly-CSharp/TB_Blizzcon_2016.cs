...
public class TB_Blizzcon_2016 : MissionEntity
{
	...
	{
		Notification.SpeechBubbleDirection bubbleDir = GetBubbleDirectionForBoss(boss);
		Vector3 quotePos = GetPositionForBoss(boss);
		switch (boss)
		{
		case BOSS.HAN:
		case BOSS.CHO:
			yield return PlayMissionFlavorLine("HanCho_Temp_BigQuote.prefab:7a7804f8f47064946bdbcfd3b78d0dac", line, quotePos, bubbleDir, 2.5f, persistCharacter);
			break;
		case BOSS.AYA:
			yield return PlayMissionFlavorLine("Aya_Temp_BigQuote.prefab:faa6811234b5e2e40b2447c7878616fe", line, quotePos, bubbleDir, 2.5f, persistCharacter);
			break;
		case BOSS.KAZAKUS:
			yield return PlayMissionFlavorLine("Kazakus_Temp_BigQuote.prefab:9d330c5f45374254181cb923722b973d", line, quotePos, bubbleDir, 2.5f, persistCharacter);
			break;
		}
		emoteThisTurn = true;
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 150:
		{
			int rand7 = Random.Range(0, 4);
			if (!hasPlayedMatchupTriggerKabal)
			{
				rand7 = Random.Range(0, 7);
			}
			Debug.Log("Potion Trigger. Random value = " + rand7);
			GameState.Get().SetBusy(busy: true);
			switch (rand7)
			{
			case 0:
		...
		case 250:
		{
			...
			{
				rand6 = Random.Range(0, 6);
			}
			Debug.Log("Jade Golem Trigger. Random value = " + rand6);
			GameState.Get().SetBusy(busy: true);
			switch (rand6)
			{
			case 0:
		...
		case 350:
		{
			...
			{
				rand5 = Random.Range(0, 8);
			}
			Debug.Log("Arms Dealing Trigger. Random value = " + rand5);
			GameState.Get().SetBusy(busy: true);
			switch (rand5)
			{
			case 0:
	...
	{
		TAG_CLASS firstPlayerClass = GameState.Get().GetFriendlySidePlayer().GetHero()
			.GetClass();
		TAG_CLASS opposingPlayerClass = GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetClass();
		switch (firstPlayerClass)
		{
	...
	private IEnumerator PlayFillLine()
	{
		...
		bool flip = (double)Random.value < 0.5;
