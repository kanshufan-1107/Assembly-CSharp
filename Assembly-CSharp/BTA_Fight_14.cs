...
public class BTA_Fight_14 : BTA_Dungeon
{
	...
	public override IEnumerator DoActionsBeforeDealingBaseMulliganCards()
	{
		...
		yield return PlayLineAlways(friendlyActor, VO_BTA_01_Female_NightElf_Mission_Fight_14_PlayerStart_01);
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			if (opposingHeroCard == "BTA_BOSS_14h")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_BTA_BOSS_14h_Male_Demon_Mission_Fight_14_BossStartImp_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (opposingHeroCard == "BTA_BOSS_14h2")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(BTA_BOSS_14h2_RustedFelReaver_Taunt, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 501:
			GameState.Get().SetBusy(busy: true);
			yield return PlayLineAlways(friendlyActor, VO_BTA_01_Female_NightElf_Mission_Fight_14_Victory_01);
			GameState.Get().SetBusy(busy: false);
			break;
		default:
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			string enemyActorCardID = GameState.Get().GetOpposingSidePlayer().GetHero()
				.GetCardId();
			GameState.Get().GetOpposingSidePlayer().GetHero()
				.GetCard()
				.GetActor();
			GameState.Get().GetFriendlySidePlayer().GetHero()
				.GetCard()
				.GetActor();
			if (cardID == "BTA_08" && enemyActorCardID == "BTA_BOSS_14h2")
			{
