...
public class AdventureMissionDisplayTray : MonoBehaviour
{
	...
	private void OnDestroy()
	{
		...
		if (ac != null)
		{
			ac.RemoveAdventureMissionSetListener(OnMissionSelected);
			ac.RemoveSubSceneChangeListener(OnSubsceneChanged);
		}
		if (GameMgr.Get() != null)
		{
	...
	public void ShowRewardsChest()
	{
		if (!(m_rewardsChest == null))
		{
			...
			bool defeatedScenario = AdventureProgressMgr.Get().HasDefeatedScenario((int)mission);
			bool hasRewards = AdventureProgressMgr.Get().ScenarioHasRewardData((int)mission) && !defeatedScenario;
			m_rewardsChest.gameObject.SetActive(hasRewards);
		}
	}
	...
	private void ShowRewards()
	{
		...
		List<RewardData> rewards = AdventureProgressMgr.Get().GetImmediateRewardsForDefeatingScenario((int)AdventureConfig.Get().GetMission());
		m_rewardsDisplay.ShowRewardsNoFullscreen(rewards, m_rewardsDisplayBone.position, m_rewardsChest.transform.position);
	}
