	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (m_popUpInfo.ContainsKey(missionEvent))
		{
			...
			if (missionEvent == 12)
			{
				...
				popup = NotificationManager.Get().CreatePopupText(UserAttentionBlocker.NONE, popUpPos, TutorialEntity.GetTextScale() * popUpScale, GameStrings.Get(m_popUpInfo[missionEvent][1]), convertLegacyPosition: false, NotificationManager.PopupTextType.FANCY);
				yield return new WaitForSeconds(3.5f);
				NotificationManager.Get().DestroyNotification(popup, 0f);
			}
			if (missionEvent == 13)
