...
public class PegCursor : MonoBehaviour
{
	...
	public void SetMode(Mode mode)
	{
		...
		if (m_currentMode == Mode.WAITING || m_currentMode == Mode.DOWNWAITING)
		{
			switch (mode)
			{
			case Mode.DOWN:
				resolvedMode = Mode.DOWNWAITING;
				break;
			case Mode.UP:
				resolvedMode = Mode.WAITING;
				break;
			default:
				resolvedMode = Mode.NONE;
				break;
			case Mode.STOPWAITING:
				break;
			}
		}
		if ((m_currentMode != Mode.DRAG || resolvedMode == Mode.STOPDRAG) && resolvedMode != Mode.NONE)
		{
			m_currentMode = resolvedMode;
		}
	}
