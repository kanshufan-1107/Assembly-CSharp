	...
	private List<InnKeepersSpecialAd> GetAllAdsFromJsonResponse(JsonNode response)
	{
		...
		try
		{
			...
			foreach (object item in responseAds)
			{
				...
				if (!string.IsNullOrEmpty(innKeepersSpecialAd.ClientVersion) && !m_forceShowIks && !StringUtils.CompareIgnoreCase(innKeepersSpecialAd.ClientVersion, "32.0"))
				{
					Log.InnKeepersSpecial.Print("Skipping IKS: {0}, mis-matched client version {0} != {1}", innKeepersSpecialAd.CampaignName, innKeepersSpecialAd.ClientVersion, "32.0");
					continue;
				}
