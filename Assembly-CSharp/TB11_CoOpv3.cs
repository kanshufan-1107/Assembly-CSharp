...
public class TB11_CoOpv3 : MissionEntity
{
	...
	private void SetUpBossCard()
	{
		if (m_bossCard == null)
		{
			int bossId = GameState.Get().GetGameEntity().GetTag(GAME_TAG.TAG_SCRIPT_DATA_ENT_1);
			Entity boss = GameState.Get().GetEntity(bossId);
			if (boss != null)
			{
				m_bossCard = boss.GetCard();
			}
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (!(m_bossCard == null))
		{
			Actor bossActor = m_bossCard.GetActor();
			switch (missionEvent)
			{
			case 2:
				GameState.Get().SetBusy(busy: true);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_BRMA17_1_RESPONSE_85.prefab:c7bbc928438b13241bde42c6578ad5c8", "VO_COOP03_03", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			case 6:
				GameState.Get().SetBusy(busy: true);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_BRMA13_1_HP_PRIEST_08.prefab:75d6f8035f037dd43af7c058f318c2fb", "VO_COOP03_06", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			case 7:
				GameState.Get().SetBusy(busy: true);
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_BRMA13_1_HP_SHAMAN_13.prefab:e248e28c2032e5c4c84490af8596f093", "VO_COOP03_07", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			case 97:
				GameState.Get().SetBusy(busy: true);
				yield return Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_NEFARIAN_NEF2_65.prefab:cad99daf56acb69428af9299fe9fb04b", "VO_COOP03_02", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			case 98:
				GameState.Get().SetBusy(busy: true);
				yield return Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_BRMA13_1_TURN1_PT1_02.prefab:ac211cc8ab665034da99720e38b6b994", "VO_COOP03_08", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			case 99:
				GameState.Get().SetBusy(busy: true);
				yield return Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeechWithCustomGameString("VO_BRMA17_1_START_78.prefab:76391ad5bad9fcb4382a2bc98d2765d7", "VO_COOP03_09", Notification.SpeechBubbleDirection.TopRight, bossActor));
				GameState.Get().SetBusy(busy: false);
				break;
			}
		}
