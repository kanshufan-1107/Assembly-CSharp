	...
	private IEnumerator BounceAnimation()
	{
		...
		for (int c = 0; c < m_BounceCount; c++)
		{
			...
			for (float i = 0f; i < 1f; i += Time.deltaTime * m_BounceSpeed)
			{
				...
				Vector3 pos = m_StartingPosition;
				float bounce = Mathf.Sin(time * (float)Math.PI);
				if (bounce < 0f)
				{
					break;
				}
				base.transform.position = new Vector3(pos.x, pos.y + bounce * m_BounceAmountOverTime, pos.z);
				yield return null;
			}
