...
public class GIL_Dungeon_Boss_38h : GIL_Dungeon
{
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		case "UNG_801":
		{
			string randomLine = PopRandomLineWithChance(m_NestingRoc);
			if (randomLine != null)
			{
		...
		case "KAR_037":
		{
			string randomLine = PopRandomLineWithChance(m_BossBirds);
			if (randomLine != null)
			{
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
			}
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_38h_Male_Goblin_Intro_01.prefab:952991d4799b8014086abf0a8eb3a4ba", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
		else if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech("VO_GILA_BOSS_38h_Male_Goblin_EmoteResponse_01.prefab:31f5ac41aa6b846419f4080611dfaf79", Notification.SpeechBubbleDirection.TopRight, enemyActor));
		}
	}
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (missionEvent == 101)
		{
			string randomLine = PopRandomLineWithChance(m_DeadBirds);
			if (randomLine != null)
			{
				yield return PlayLineOnlyOnce(enemyActor, randomLine);
			}
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		case "KAR_037":
			...
			{
				string randomLine = PopRandomLineWithChance(m_PlayerBirds);
				if (randomLine != null)
				{
					yield return PlayLineOnlyOnce(enemyActor, randomLine);
				}
