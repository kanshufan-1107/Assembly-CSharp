...
public class WingDbfRecord : DbfRecord
{
	...
	[SerializeField]
	private EventTimingType m_requiredEvent = DbfShared.GetEventMap().ConvertStringToSpecialEvent("none");
	...
	public EventTimingType RequiredEvent => m_requiredEvent;
	...
	public override void SetVar(string name, object val)
	{
		...
		case "UNLOCK_ORDER":
			...
			break;
		case "REQUIRED_EVENT":
			m_requiredEvent = DbfShared.GetEventMap().ConvertStringToSpecialEvent((string)val);
			break;
	...
	{
		WingDbfAsset dbfAsset = DbfShared.GetAssetBundle().LoadAsset(resourcePath, typeof(WingDbfAsset)) as WingDbfAsset;
		if (dbfAsset == null)
		{
			records = new List<T>();
