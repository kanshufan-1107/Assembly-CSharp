...
public class KAR05_Wolf : KAR_MissionEntity
{
	...
	private Actor GetDireWolf()
	{
		...
		foreach (Card card in player.GetBattlefieldZone().GetCards())
		{
			...
			if (cardEntity.GetControllerId() == player.GetPlayerId() && cardEntity.GetCardId() == "EX1_162")
			{
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		string cardID = "ENEMY_" + entity.GetCardId();
		if (m_playedLines.Contains(cardID))
		{
			yield break;
		}
		m_playedLines.Add(cardID);
		if (cardID == "KARA_05_02")
		{
			GameState.Get().SetBusy(busy: true);
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "EX1_162"))
		{
			...
			{
				GameState.Get().SetBusy(busy: true);
				yield return PlayEasterEggLine(enemyActor, "VO_BigBadWolf_Male_Worgen_WolfScarletCrusader_01.prefab:dcdcc54bc1a75374baf3a5237d0a7141");
				GameState.Get().SetBusy(busy: false);
			}
			yield break;
		}
		...
		{
			GameState.Get().SetBusy(busy: true);
			yield return PlayEasterEggLine(enemyActor, "VO_BigBadWolf_Male_Worgen_WolfDireWolfAlpha_01.prefab:d4457c645514a6b49be8345218d13cf6");
