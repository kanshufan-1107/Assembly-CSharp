...
public class ULDA_Dungeon_Boss_16h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_16h_Male_Ogre_IntroEliseResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else if (playerHeroCardID == "ULDA_Finley")
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_16h_Male_Ogre_IntroFinleyResponse_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_180"))
		{
			if (cardID == "ULD_206")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_16h_Male_Ogre_PlayerTrigger_Restless_Mummy_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_16h_Male_Ogre_PlayerTrigger_Sunstruck_Henchman_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!m_playedLines.Contains(entity.GetCardId()) || entity.GetCardType() == TAG_CARDTYPE.HERO_POWER)
		{
			...
			{
			case "ULD_193":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_16h_Male_Ogre_BossLivingMonument_01);
				break;
			case "ULD_186":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_16h_Male_Ogre_BossPharoahCat_01);
				break;
			case "ULD_194":
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_16h_Male_Ogre_BossWastelandScorpid_01);
				break;
			}
		}
	}
