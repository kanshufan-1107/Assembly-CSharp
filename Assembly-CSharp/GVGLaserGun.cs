...
public class GVGLaserGun : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		m_minAngleIndex = 0;
		m_maxAngleIndex = 0;
		float minAngle = m_AngleDefs[0].m_Angle;
		float maxAngle = minAngle;
		for (int j = 0; j < m_sortedAngleDefIndexes.Count; j++)
		{
			AngleDef def3 = m_AngleDefs[m_sortedAngleDefIndexes[j]];
			if (def3.m_Angle < minAngle)
			{
				minAngle = def3.m_Angle;
				m_minAngleIndex = j;
			}
			if (def3.m_Angle > maxAngle)
			{
				maxAngle = def3.m_Angle;
				m_maxAngleIndex = j;
			}
			if (def3.m_Default)
			{
				m_angleIndex = j;
				SetAngle(def3.m_Angle);
			}
		}
	}
	...
	private AngleDef GetAngleDef()
	{
		...
		int angleIndex = m_sortedAngleDefIndexes[m_angleIndex];
		return m_AngleDefs[angleIndex];
	}
	...
	{
		AngleDef targetDef = GetAngleDef();
		float angle = Mathf.MoveTowards(m_angle, targetDef.m_Angle, m_RotationSpeed * Time.deltaTime);
		if (angle <= targetDef.m_Angle)
		{
			if (m_requestedRotationDir == 0 || m_angleIndex == m_minAngleIndex)
			{
				SetAngle(angle);
	...
	{
		if (m_rotationDir == 0 && !m_leverEffectsActive && InputCollection.GetMouseButtonUp(0) && IsOver(m_GunLever))
		{
			PullLever();
		}
	}
