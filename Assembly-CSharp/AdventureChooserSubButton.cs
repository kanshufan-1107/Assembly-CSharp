...
public class AdventureChooserSubButton : ChooserSubButton
{
	...
	{
		int progressCount = 0;
		AdventureDataDbfRecord dataRecord = GameUtils.GetAdventureDataRecord((int)m_TargetAdventure, (int)m_TargetMode);
		if (dataRecord != null && dataRecord.ShowPlayableScenariosCount)
		{
			progressCount = ((m_TargetMode != AdventureModeDbId.CLASS_CHALLENGE) ? AdventureProgressMgr.Get().GetNumPlayableAdventureScenarios(m_TargetAdventure, m_TargetMode) : AdventureProgressMgr.Get().GetPlayableClassChallenges(m_TargetAdventure, m_TargetMode));
		}
		...
		if (m_progressCounter != null)
		{
			if (progressCount > 0)
			{
				...
				m_progressCounterText.Text = progressCount.ToString();
			}
			else
			{
				m_progressCounter.SetActive(value: false);
