...
public class ULDA_Dungeon_Boss_53h : ULDA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			...
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_ULDA_BOSS_53h_Female_Vulpera_IntroSpecial_Finley_01, Notification.SpeechBubbleDirection.TopRight, enemyActor));
			}
			else
			{
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		if (missionEvent == 101)
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_53h_Female_Vulpera_BossTriggerWastelandSapper_01);
		}
		else
		{
	...
	protected override IEnumerator RespondToFriendlyPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "ULD_191"))
		{
			if (cardID == "ULD_170")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_53h_Female_Vulpera_PlayerTrigger_Weaponized_Wasp_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_53h_Female_Vulpera_PlayerTrigger_Beaming_Sidekick_01);
		}
	}
	...
	protected override IEnumerator RespondToPlayedCardWithTiming(Entity entity)
	{
		...
		if (!(cardID == "DAL_728"))
		{
			if (cardID == "ULD_282")
			{
				yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_53h_Female_Vulpera_BossTriggerJarDealer_01);
			}
		}
		else
		{
			yield return PlayLineOnlyOnce(enemyActor, VO_ULDA_BOSS_53h_Female_Vulpera_BossTriggerDaringEscape_01);
		}
	}
}
