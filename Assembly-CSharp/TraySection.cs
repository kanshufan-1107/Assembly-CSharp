...
public class TraySection : MonoBehaviour
{
	...
	private IEnumerator MoveToOriginalPosition(float time, DelOnDoorStateChangedCallback callback = null)
	{
		...
		_ = base.transform.GetChild(0).position;
		Vector3 goalPosition;
		while (timeLive < time)
		{
			goalPosition = m_parent.position;
			goalPosition.y += 3.238702f;
			m_deckBox.transform.position = Vector3.Lerp(startPos, goalPosition, timeLive / time);
			timeLive += Time.deltaTime;
			yield return 0;
		}
		goalPosition = m_parent.position;
		goalPosition.y += 3.238702f;
		m_deckBox.transform.position = goalPosition;
	...
	{
		UIBScrollableItem scrollableItem = GetComponent<UIBScrollableItem>();
	...
	private void Awake()
	{
		...
		UIBScrollableItem scrollItem = GetComponent<UIBScrollableItem>();
		if (scrollItem != null)
		{
			scrollItem.SetCustomActiveState(IsDeckBoxShown);
		}
	...
	private void OpenDoor(bool isImmediate, DelOnDoorStateChangedCallback callback, object callbackData)
	{
		...
		m_isOpen = true;
		Animation doorAnimation = m_door.GetComponent<Animation>();
		doorAnimation[DOOR_OPEN_ANIM_NAME].time = (isImmediate ? doorAnimation[DOOR_OPEN_ANIM_NAME].length : 0f);
		doorAnimation[DOOR_OPEN_ANIM_NAME].speed = 6f;
	...
	private void CloseDoor(bool isImmediate, DelOnDoorStateChangedCallback callback, object callbackData)
	{
		...
		m_isOpen = false;
		Animation doorAnimation = m_door.GetComponent<Animation>();
		doorAnimation[DOOR_CLOSE_ANIM_NAME].time = (isImmediate ? doorAnimation[DOOR_CLOSE_ANIM_NAME].length : 0f);
		doorAnimation[DOOR_CLOSE_ANIM_NAME].speed = 6f;
	...
	private void PlayDoorAnimation(string animationName, DelOnDoorStateChangedCallback callback, object callbackData)
	{
		...
		OnDoorStateChangedCallbackData onDoorStateChangedCallbackData = new OnDoorStateChangedCallbackData
		{
			m_callback = callback,
		...
		StartCoroutine("WaitThenCallDoorAnimationCallback", onDoorStateChangedCallbackData);
	}
	...
	private IEnumerator WaitThenCallDoorAnimationCallback(OnDoorStateChangedCallbackData callbackData)
	{
		...
		{
			Animation doorAnimation = m_door.GetComponent<Animation>();
			yield return new WaitForSeconds(doorAnimation[callbackData.m_animationName].length / doorAnimation[callbackData.m_animationName].speed);
